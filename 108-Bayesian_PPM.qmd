...
Esta prueba está basada en la simulación de la distribución posterior. Se calcula la distribución de las $\lambda$ y se determina si la $\lambda$ observada es significativamente más grande que 1. También podemos calcular algunas estadísticas descriptivas, donde `p_increase` es la probabilidad de que $\lambda > 1$. En este caso, el valor de $\lambda$ no es significativamente más grande que 1, ya que se obtuvo un valor de 0.14. Aunque el histograma sugiere que el crecimiento es menor que 1, la probabilidad de que $\lambda$ sea mayor que 1 es de 0.14, lo que indica que hay una probabilidad del 14% de que la población esté creciendo, dando poco apoyo a que la población esté creciendo.
