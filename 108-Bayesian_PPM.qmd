...
Consideremos una especie hipotética con tres etapas en su ciclo de vida: semillas, plántulas y adultos. En este caso, solo los adultos producen semillas. El primer paso del análisis consiste en identificar qué etapas tienen mayor influencia en la dinámica poblacional y cómo optimizar la recolección de datos en campo.
...
Estas interpretaciones extremas —que ningún individuo muere o que todos mueren— no son biológicamente realistas. Sin embargo, pueden surgir como resultado de un tamaño de muestra reducido. Por ejemplo, si solo se observaron 4 plantas adultas (debido a que la especie es rara) y todas sobrevivieron, se estimaría una tasa de supervivencia del 100%, lo que implicaría que los individuos son "inmortales". Este resultado no refleja la realidad biológica, sino una limitación del muestreo: por simple azar, no se observó ninguna muerte. Por lo tanto, es fundamental reconocer cuándo los parámetros estimados no representan adecuadamente la historia de vida típica de la especie.
...
2.  **Es reducible**: el grafo del ciclo de vida no es fuertemente conexo, es decir, hay estadios desde los cuales no se puede alcanzar a todos los demás, de modo que la matriz puede reordenarse en bloques triangulares. Suele ser un artefacto del muestreo o de una estructura biológica mal representada.
...
![*Lepanthes eltoroensis*. Foto: Edwin Guevara. Estudio demográfico: @tremblay2003population.](images/Lepanthes_eltoroensis_Edwin_Guevara.jpg){#fig-lepanthes-eltoroensis-edwin-guevara}

![*Lepanthes eltoroensis*. Foto: Edwin Guevara. Estudio demográfico: @tremblay2003population.](images/Lepanthes_eltoroensis_Phorophyte_Edwin_Guevara.jpg){#fig-lepanthes-eltoroensis-phorophyte-edwin-guevara}
:::
...
Supongamos que tiene una base de datos llamada `L_elto` con las siguientes columnas:
...
TF # Esta es la estructura de etapas de vida para esa población. Note que tenemos dos matrices, una de transiciones **T** y otra de fecundidad **F**.
```
...
::: callout-important
**El "peso de la previa" es un tamaño de muestra equivalente.** Asignar `priorweight = 0.5` con $N = 2$ observaciones es como decir: "antes de salir al campo, mi creencia tiene la misma fuerza que $0.5 \times 2 = 1$ observación adicional". Si su previa tiene peso efectivo 5 y observa 50 transiciones, los datos pesarán 10 veces más que la previa. La regla práctica es dejar que los datos dominen: el peso de la previa debería ser **mucho menor** que el tamaño de muestra observado, salvo que tengas razones biológicas muy fuertes para confiar en la previa.
:::
...
- `priorweight = 1`: la previa pesa lo mismo que los datos. Solo justificado si los datos son muy escasos y la previa es muy confiable.
- `priorweight > 1`: la previa pesa más que los datos. Rara vez justificado en estudios poblacionales.

&nbsp;

::: callout-important
**El peso de la previa es proporcional a $N$, así que no es el mismo en todos los estadios.**

Como `priorweight` multiplica al tamaño de muestra, un mismo valor produce pesos muy distintos según cuántos individuos se hayan observado en cada estadio. Con `priorweight = 0.1`, un estadio con 100 individuos recibe una previa que vale 10 observaciones, y otro con 3 individuos recibe una que vale 0.3.

El efecto es contraintuitivo: **la previa pesa menos justamente donde los datos son más escasos**, que es donde más falta hace. Si el objetivo es sostener los estadios raros, conviene comprobar el peso resultante estadio por estadio, no solo elegir un valor global. En el capítulo de [Métodos de simulaciones](115-Metodos_de_simulaciones.qmd) hay un caso trabajado en el que ese peso va de 0.02 a 0.99 dentro de la misma matriz.

Recuerde además que `priorweight` no altera la forma de la previa, solo su influencia. Una previa mal razonada no mejora por pesar poco.
:::
...
- Es ergódica e irreducible: todas las etapas están conectadas directa o indirectamente, por lo que la dinámica asintótica ya no depende de la estructura inicial.
...
Supongamos que estamos analizando las transiciones desde la etapa de plántula (s). En estos datos se observaron `r N[1]` transiciones desde ese estadio. Como `priorweight` es un **multiplicador sobre el tamaño de muestra observado**, con `priorweight = 0.1` la previa aporta `r round(0.1 * N[1], 1)` observaciones equivalentes y el tamaño de muestra efectivo es `r round(1.1 * N[1], 1)`. Aun así el intervalo de credibilidad resulta amplio, lo que refleja la alta incertidumbre de la estimación. Por ejemplo:
...
```{r}
#| label: tbl-bayes-icr
#| tbl-cap: "Estimación puntual e intervalo de credibilidad del 95 % para los destinos posibles de un individuo que estaba en el estadio de plántula, con `priorweight = 0.1`. La última fila es la mortalidad."
...
# Se rotula cada fila con su destino: sin nombres, la tabla era una sola
# columna llamada «x» y no se sabía a qué estadio correspondía cada valor.
data.frame(
  Destino = c("Plántula (s)", "Juvenil (j)", "Adulto (a)", "Muere"),
  Estimacion = sprintf("%.4f", p),
  ICr = sprintf("(%.4f, %.4f)", lcl, ucl)
) |>
  flextable() |>
  set_header_labels(
    Destino = "Destino en t+1",
...
Cuando se aumenta el **tamaño de muestra efectivo**, los **intervalos de credibilidad** se vuelven más estrechos, lo que indica mayor confianza en las estimaciones puntuales. Si se especifica `priorweight = 9`, la previa aporta $9 \times$ `r N[1]` $=$ `r 9 * N[1]` observaciones equivalentes y el tamaño de muestra efectivo pasa a `r 10 * N[1]`, de modo que los intervalos de credibilidad se estrechan mucho. Conviene advertir que este caso es deliberadamente extremo: la previa pesa nueve veces más que los datos, es decir, lo que se muestra ya no es la información del campo sino la de la previa. Se incluye para ilustrar el efecto del tamaño de muestra efectivo, no como una recomendación. Entonces, es importante enfatizar en que el tamaño de muestra tiene gran impacto sobre la confianza que se tiene sobre el estimador puntual (el promedio) de las transiciones, permanencias y mortalidad. Note en la figura como el tamaño del intervalo de credibilidad cambia drásticamente cuando se aumenta el número de datos.
...
```{r}
#| label: fig-bayes16
#| fig-cap: "Probabilidades posteriores de transición por destino para dos pesos de la previa (priorweight = 0.1 y 9), representadas con puntos y barras de error del intervalo de credibilidad del 95%."
...
alltogethernow <- tibble(
  priorweight = factor(rep(c(0.1, 9), each = 4)), # 0.1 es el peso usado en el bloque anterior
  fate = factor(
...
Siguiendo el marco bayesiano que hemos venido trabajando en este capítulo, es necesario especificar las previas de la fecundidad como una matriz. Las celdas de la matriz de los estados en los que no hay reproducción deben ser `NA`, usando `NA_real_`, el cual es un valor que no está presente pero podría aceptar valores con puntos decimales. Conviene detenerse en la parametrización. La previa Gamma se especifica con dos matrices, `alpha` y `beta`, y en la parametrización *forma–tasa* que usa `raretrans` la **media previa de la fecundidad es** $\alpha/\beta$, mientras que $\beta$ hace las veces de **tamaño de muestra previo**. Con $\alpha = \beta = 10^{-4}$, la media previa es $10^{-4}/10^{-4} = 1$, no $0.0001$: lo que vale $10^{-4}$ es el **peso** de esa previa, es decir, es una previa prácticamente sin información. Por eso el resultado que sigue queda dominado por los datos observados.
...
Ahora simulamos 5000 veces, calculamos el valor $\lambda$ de cada matriz y creamos un histograma de su distribución. Se añaden los valores de $\lambda$ en un objeto llamado `RLT_0.5`. Se visualiza la distribución de $\lambda$ usando un histograma. Nótese que en este bloque la previa de la fecundidad **cambia** respecto de la del bloque anterior: se pasa de $\alpha = \beta = 10^{-4}$ a $\alpha_2 = 0.025$ y $\beta_2 = 1$. La media previa sigue siendo pequeña ($\alpha_2/\beta_2 = 0.025$), pero ahora la previa **sí pesa**, equivalente a una observación ($\beta_2 = 1$). El cambio es deliberado: con la previa anterior, prácticamente sin peso, las simulaciones de fecundidad resultaban demasiado dispersas. Note que el valor de $\lambda = 1$ queda dentro de la distribución; en otras palabras, el histograma muestra que la mayoría de los valores de $\lambda$ están cerca de 1.

&nbsp;

::: callout-warning
**La probabilidad posterior depende de la previa que se declare.** El valor de `p_increase` que se obtiene más adelante procede de estas dos matrices, `alpha2` y `beta2`, y no solo de los datos. Cambiar $\alpha_2$ o $\beta_2$ desplaza la distribución posterior de $\lambda$ y, con ella, la probabilidad de crecimiento. Es un buen ejercicio volver a correr este bloque con la previa del apartado anterior ($\alpha = \beta = 10^{-4}$) y comparar el resultado: comprobar esta sensibilidad forma parte de cualquier análisis bayesiano bien hecho, y conviene reportar siempre la previa empleada junto con el resultado.
:::
...
A diferencia de un contraste de hipótesis frecuentista (donde se obtiene un valor-p), el marco bayesiano nos permite responder directamente la pregunta de interés: **¿con qué probabilidad la población está creciendo?** Lo hacemos contando qué proporción de las simulaciones de la distribución posterior produjo $\lambda > 1$. En este caso, con la previa declarada arriba ($\alpha_2 = 0.025$, $\beta_2 = 1$, `priorweight = 0.5`), $P(\lambda > 1 \mid \text{datos}, \text{previas}) = 0.14$, lo que indica que solo en el 14% de los escenarios consistentes con los datos y la previa la población crece. La evidencia para crecimiento poblacional es débil; lo más probable es que la población esté en declive. La estadística `p_increase` reporta exactamente este valor.
...
**Siempre construye previas informadas** que reflejen el ciclo de vida de la especie: marca con 0 las transiciones imposibles, asigna probabilidades pequeñas a las raras pero posibles, y concentra masa en las transiciones que la biología espera. La regla práctica es: si no puede defender la previa frente a un especialista de la especie, no la use.

La sección que sigue muestra qué pasa con una previa uniforme **únicamente con fines didácticos** — para que se vea de primera mano por qué falla. No la copie en un análisis de su propia especie.
:::
...
Note que no se recomienda el uso de una previa uniforme; aquí se utiliza solo para mostrar el concepto. Una distribución previa uniforme no refleja un ciclo de vida razonable para especies biológicas. Por ejemplo, una previa uniforme podría hacer que las plántulas pasen a adulto de un tiempo a otro o que los adultos regresen a ser plántulas, lo cual no tiene mucho sentido biológico en muchas especies. Considere lo siguiente: ¿es posible que una ballena bebé pueda pasar a ser madre al año siguiente? Añadir una transición de 0.25 de ballena bebé a madre no tiene sentido biológico.
