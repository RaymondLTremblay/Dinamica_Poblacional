---
description: "Análisis de elasticidad y sensibilidad: qué transiciones del ciclo de vida tienen mayor impacto proporcional sobre la tasa de crecimiento poblacional λ."
...
Por: Demetria Mondragón, Ernesto Mujica Benitez y Elaine González
...
## Elasticidad poblacional: por qué pequeñas variaciones importan
...
#| label: tbl-elas1
#| tbl-cap: 'Matriz de elasticidad de *Erycina crista-galli*. En las casillas del renglón "suma" se encuentra el resultado de la sumatoria de los valores de elasticidad de cada entrada de la matriz por estadio (es decir, todos los valores de la columna).'
...
Los valores anteriores son producidos por `popbio::elas()`, que internamente combina los vectores propios derecho e izquierdo de la matriz con el producto de Hadamard. Para ilustrar de dónde sale un valor de elasticidad, podemos reconstruirlos paso a paso a partir de los vectores propios. La sensibilidad se obtiene de:

$$s_{ij} = \frac{v_i\, w_j}{\langle v, w \rangle}$$

donde $\mathbf{w}$ es el vector propio derecho dominante (estructura estable de estadios), $\mathbf{v}$ el vector propio izquierdo dominante (valor reproductivo) y $\langle v, w \rangle = \sum_k v_k w_k$. La matriz de elasticidad se obtiene como $\mathbf{E} = (1/\lambda)\,\mathbf{S} \circ \mathbf{A}$ [@caswell2001matrix].

```{r ElasManual, message=FALSE, warning=FALSE}

# Paso 1: valor propio dominante y vectores propios
ev <- eigen(ec)
lambda <- Re(ev$values[1]) # tasa finita de crecimiento (lambda)
w <- Re(ev$vectors[, 1]) # vector propio derecho: estructura estable

ev_t <- eigen(t(ec))
v <- Re(ev_t$vectors[, 1]) # vector propio izquierdo: valor reproductivo
...
```{r}
#| label: tbl-elas2
#| tbl-cap: "Matriz de elasticidad de *Erycina crista-galli*. Los colores de las entradas de la matriz representan los procesos o tasas vitales: rojo es la permanencia, azul el crecimiento, el amarillo el decrecimiento o retrogresión y el verde la fecundidad. Así, para obtener el valor de elasticidad por proceso, se suman las entradas del color que lo representen."
...
Las elasticidades también han sido utilizadas para establecer el comportamiento demográfico de las especies, lo que ha permitido generar patrones entre formas de vida, así como comparaciones tanto entre formas de vida como entre especies [@mondragon2015population; @silvertown1996interpretation]. Así, por ejemplo, si quisiéramos comparar el comportamiento demográfico de las orquídeas de ramillas versus las orquídeas epífitas convencionales (@tbl-elas3) (reconstruido de [@mondragon2007life]), veríamos que para ambos tipos de orquídeas lo más importante es la permanencia (supervivencia) de los individuos, en tanto que se puede observar una diferencia entre la orquídea de ramilla y las otras, en la contribución de los estadios al comportamiento de lambda, ya que para la primera, los individuos reproductivos presentan mayores valores de elasticidad (al igual que para *Lepanthes eltoroensis*), en tanto que para las otras orquídeas los valores más altos de elasticidad los tienen los individuos no reproductivos [@mondragon2007life].

En la @fig-Elas7 se observan los valores de elasticidad añadidos por proceso de historia de vida para cinco especies de orquídeas epífitas:

::: img-float
![*Lepanthes eltoroensis*. Foto: Tremblay. Estudio demográfico: @tremblay2003population.](images/Lepanthes_eltoroensis_Tremblay.jpeg){#fig-lepanthes-eltoroensis-tremblay style="float: left; margin: 20px;width: 40% ; "}
:::
...
```{r}
#| label: tbl-elas3
#| tbl-cap: "Valores de elasticidad por proceso y por estadios no reproductivos y reproductivos de cinco especies de orquídeas epífitas (*Erycina crista-galli* considerada orquídea de ramilla). La fila de *Erycina crista-galli* se calculó a partir de la matriz presentada antes en este mismo capítulo, de modo que coincide con los valores del texto; la columna «Crecimiento» agrupa crecimiento y retrogresión. Las demás filas provienen de sus publicaciones de origen."
#| echo: false
...
::: img-float
![*Lepanthes caritensis*. Foto: Edwin Guevara. Estudio demográfico: @tremblay1997lepanthes.](images/Lepanthes_caritensis_Edwin_Guevara.jpg){#fig-lepanthes-caritensis-edwin-guevara style="float: left; margin: 10px;width: 50%; "}
:::

::: img-float
![*Lepanthes caritensis*. Foto: Edwin Guevara. Estudio demográfico: @tremblay1997lepanthes.](images/Lepanthes_caritensis_Phorophyte.jpg){#fig-lepanthes-caritensis-phorophyte style="float: right; margin: 10px;width: 50%;"}
:::
...
#| label: fig-Elas7
#| fig-cap: "Diagrama de barras agrupadas de la elasticidad por proceso demográfico (fecundidad, estasis, crecimiento), con barras coloreadas por especie."
EE <- Elasticidad_s %>%
...
En la @fig-Elas8 se observan los valores de elasticidad agrupados por estadios (plántulas, individuos no reproductivos y reproductivos). Los valores de elasticidad son más altos en los estadios no reproductivo en tres especies y reproductivos (adultos activos) en dos especies. Las plántulas tienen las elasticidades más pequeñas en cuatro de las cinco especies. La excepción es *L. eltoroensis*, donde superan a las de los individuos no reproductivos (0.113 frente a 0.078).

```{r, message=FALSE}
#| label: fig-Elas8
#| fig-cap: "Diagrama de barras agrupadas de la elasticidad por etapa de vida (plántulas, no reproductivos, reproductivos), con barras coloreadas por especie."
...
Capturar los datos de la matriz para *Erycina crista-galli* [@mondragon2007life] (tabla 4 dentro de dicho artículo)
