---
description: "Diseño y representación gráfica de ciclos de vida en plantas, paso previo a construir matrices de proyección poblacional. Ejemplos con orquídeas neotropicales."
image: "images/CV_5.png"
open-graph:
  description: "Diseño y representación gráfica de ciclos de vida en plantas, paso previo a construir matrices de proyección poblacional. Ejemplos con orquídeas neotropicales."
twitter-card:
  description: "Diseño y representación gráfica de ciclos de vida en plantas, paso previo a construir matrices de proyección poblacional. Ejemplos con orquídeas neotropicales."
...
```{r CV1-setup-diagrams, message=FALSE, warning=FALSE, tidy=FALSE}
#| docx_fit: true

# Instalar (si es necesario) y cargar
...
# Carpeta de salida para PNGs. Los png_name pasados a render_dual()
# ya incluyen el prefijo "images/", así que out_dir se queda en la raíz
# del proyecto. Esto asegura que las figuras queden disponibles en
# images/ para el productor del libro.
out_dir <- "."
...
# Helper: DOT -> PNG (+ sibling PDF vectorial para el editor)
grviz_dot_to_png <- function(dot, outfile, width = PNG_WIDTH, height = PNG_HEIGHT) {
  ...
  # PDF vectorial sibling (mismo nombre, extensión .pdf). Permite que
  # `figuras_editor/` reciba el diagrama en formato vectorial.
  pdf_out <- sub("\\.[^./\\\\]+$", ".pdf", outfile)
  try(rsvg::rsvg_pdf(charToRaw(svg), pdf_out), silent = TRUE)
  outfile
}
...
# Helper: SIEMPRE guarda el PNG en images/ (para el productor del libro),
# además de mostrar el widget interactivo en HTML o el PNG estático en PDF.
render_dual <- function(dot, png_name) {
  ...
  # Generar el PNG siempre, así el editor tiene acceso al archivo
  # independientemente del formato en que se renderice el libro.
...
**Dibuja el ciclo de vida antes de salir al campo.** Elaborar un diagrama hipotético del ciclo de vida —con todas las etapas plausibles y las transiciones entre ellas— antes de iniciar el muestreo permite identificar qué datos hacen falta, qué etapas son difíciles de observar y qué transiciones son raras. Sin esa hoja de ruta, es fácil regresar del campo con un conjunto de datos incompleto que no permite construir una matriz biológicamente coherente.
...
## Ejemplo de ciclo de vida sencillo
...
#| docx_fit: true
#| label: fig-CV_2
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de tres estadios (semillas, plántulas, adultos) para una especie hipotética."
...
**Convención de colores en los diagramas de este capítulo.**

- **Verde:** crecimiento (transición a una etapa más avanzada)
- **Rojo:** permanencia (mismo estadio en el siguiente intervalo)
- **Amarillo:** reproducción (producción de semillas)
- **Violeta:** regresión (transición a una etapa más temprana)
- **Azul:** entrada o permanencia en latencia vegetativa
...
#| docx_fit: true
#| label: fig-CV_5
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de cinco estadios (semillas, plántulas y tres clases de adultos) para una especie hipotética."
...
cols <- c(
  "PaleGreen4", "PaleGreen4", "red", "PaleGreen4", "Goldenrod1", "red", "PaleGreen4",
  "Goldenrod1", "red"
)
...
```{r}
#| docx_fit: true
#| label: fig-CV_6Telipogon_helleri
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de tres estadios (inmaduros, reproductivos pequeños y grandes) para *Telipogon helleri*."
...
cols <- c(
  "red", "PaleGreen4", "PaleGreen4", "Goldenrod1", "red", "PaleGreen4",
...
**Ausencia no es muerte.** En orquídeas terrestres con latencia vegetativa, la ausencia de una planta marcada en un censo **no** implica que haya muerto: el tubérculo subterráneo puede estar vivo y reaparecer años después. Asumir mortalidad cuando hay latencia subestima la supervivencia y sesga las matrices de transición. Cuando se sospecha latencia, conviene usar métodos bayesianos para estimar la probabilidad de que un individuo "ausente" siga vivo [@tremblay2009dormancy].
...
```{r}
#| docx_fit: true
#| label: fig-CV7Prasophyllum_correctum
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de cuatro estadios incluyendo latencia para *Prasophyllum correctum*."
...
cols <- c(
  "black", "red", "palegreen4", "black", "goldenrod1", "blue", "red", "black",
  "blue", "blue", "red"
)
...
#### *Ophrys sphegodes* {.unnumbered}

En el trabajo de Waite y Hutchings [@waite1991effects] con *Ophrys sphegodes* se recolectaron datos de la etapa vegetativa, plantas en flor y latencia de uno y dos años; por consecuencia, la historia de vida está representada por 4 etapas. Los datos provienen de la figura 1 y la tabla 3b (periodo de herbivoría por oveja). En este caso, se hacen análisis para determinar cuál es la probabilidad de que un individuo con latencia vegetativa múltiple (uno o dos años) esté vivo, al comparar poblaciones que están sujetas a la herbivoría por ovejas o ganado. Una síntesis detallada del ciclo de vida, la ecología y la demografía de *Ophrys sphegodes* está disponible en su tratamiento dentro de la serie *Biological Flora of the British Isles* [@jacquemyn2015biological].

Dado que el análisis se enfoca en la supervivencia, en el estudio no se incluye la reproducción (no hay línea amarilla). Las etapas que pasen a latente el siguiente año se señalan en azul, las que se quedan en la misma etapa están en rojo, mientras que en verde el tránsito a una etapa no latente.

```{r}
#| docx_fit: true
#| label: fig-CV8_Ophrys
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de cuatro estadios incluyendo latencia de dos años para *Ophrys sphegodes*."

G1 <- matrix(
  c(
    0.0080, 0.0184, 0.0149, 0.0148,
...
cols <- c(
  "red", "PaleGreen4", "blue", "PaleGreen4", "red", "blue",
...
#### *Cypripedium parviflorum*, *Epipactis atrorubens* y *Orchis purpurea* {.unnumbered}

::: img-float

![*Orchis purpurea*. Foto: Hans Jacquemyn. Estudio demográfico: @jacquemyn2010seed — la limitación por semillas restringe el crecimiento poblacional en poblaciones sombreadas.](images/Orchis_purpurea_1_Hans_Jacquemyn.jpg){#fig-orchis-purpurea-1-hans-jacquemyn style="float: left; margin: 10px;width: 50% ; "}

:::

Las dimensiones de la matriz y la cantidad de etapas o años incluidos en un análisis depende de múltiples factores. Cuando se trabaja con datos de campo, el acceso a individuos de cada etapa del ciclo de vida es, sin duda, un factor determinante. En la revisión de las publicaciones sobre orquídeas se encuentran análisis matriciales que varían entre 3 y 29 etapas de desarrollo, en promedio 5.6 etapas (desv. estándar de 3.2). El trabajo con mayor número de etapas es el de [@shefferson2014life], que utilizó 29 en *Cypripedium parviflorum* var. *parviflorum*.

El costo de pasar a una fase latente después de diferentes etapas se evaluó en *Epipactis atrorubens*. En este análisis se definieron 11 estados, considerando la posibilidad de entrar en latencia vegetativa desde todas estas fases emergentes [@jakalaniemi2011orchids]. En *Orchis purpurea*, los autores modelaron el diagrama del ciclo de vida, definiendo 6 etapas de desarrollo: protocormos, tubérculos, plántulas, juveniles (más de un año y menos de 3 hojas), plantas sin flores (más de 3 hojas) y plantas con flores, a continuación se presenta la matriz correspondiente, tomada de [@jacquemyn2010seed]. Las transiciones a una etapa subsiguiente se señalan en verde, en violeta las transiciones a una etapa más pequeña y la reproducción en amarillo. En este ciclo de vida las etapas de protocormos, tubérculos y plántulas son de un año cada una. Si estos individuos no crecen a la próxima etapa se supone que fallecieron.
...
#| docx_fit: true
#| label: fig-CV9_Op
#| fig-cap: "Diagrama del ciclo de vida con matriz de proyección de seis estadios (protocormos, tubérculos, plántulas, juveniles, adultos sin y con flores) para *Orchis purpurea*."
...
cols <- c(
  "PaleGreen4", "PaleGreen4", "PaleGreen4", "red", "PaleGreen4",
...
render_dual(dot, "images/CV9_Op.png")
```

![*Orchis purpurea*. Foto: Hans Jacquemyn. Estudio demográfico: @jacquemyn2010seed.](images/Orchis_purpurea_2_Hans_Jacquemyn.jpg){#fig-orchis-purpurea-2-hans-jacquemyn fig-align="center" width="60%"}
...
**Tres errores típicos en datos de campo —y por qué importan.**

- **Etapas sin mortalidad** (suma de columna = 1 sin pérdidas): biológicamente irreal salvo en casos de latencia perfecta.
- **Etapas con mortalidad total** (todos los individuos mueren entre intervalos): rompe la cadena del ciclo y borra una transición real que probablemente sí ocurre con baja frecuencia.
- **Permanencia perpetua** (un estadio nunca contribuye a otros ni muere): elimina toda dinámica del estadio.

Estos errores suelen aparecer cuando el tamaño de muestra es pequeño o las transiciones son raras. Antes de aceptar la matriz, conviene revisar el diagrama del ciclo de vida y preguntarse: ¿es biológicamente plausible que esta entrada sea exactamente cero?

:::

&nbsp;

La importancia de este capítulo radica en reconocer la diferencia entre la historia de vida real de un organismo y la información que se puede recolectar en el campo. En los estudios poblacionales de orquídeas, dos desafíos en su investigación y muestreo son la dificultad, por un lado, de rastrear y hacer un seguimiento de las semillas en el campo y, por otro, el determinar los sitios adecuados para su germinación (sitios seguros). Este segundo aspecto implica considerar la presencia de micorrizas, cuya asociación con las semillas es determinante para su germinación. Estas etapas tempranas del ciclo de vida de las orquídeas suelen estar rodeadas de gran incertidumbre, lo que limita los análisis e interpretaciones de su evolución y conservación.

Además de los anteriores, existen otros problemas. La recolección de datos en campo puede ser irreal en el contexto de la historia de vida y la base matemática del modelo representado. Para más detalles ver el capítulo [Impacto de datos sin sentido](122-Impacto_de_Datos_sin_Sentido.qmd). Por ejemplo, en la @fig-cv10 aparecen tres errores comunes durante la recolección de datos de la historia de vida de una especie. El primero es que no hay mortalidad de los individuos en una de las etapas (juveniles), el segundo es que todos los individuos se mueren (plántulas) y el tercero es que los individuos se mantienen en la misma etapa de desarrollo (adultos) y no hay muerte. Dada la muerte de las plántulas, no hay flecha que una a este estado con otros estados. La falta de continuidad o conexión entre las etapas del ciclo de vida es un error que ocurre típicamente cuando el tamaño de muestra es pequeño o la probabilidad de transición es rara o poco frecuente; por ejemplo, los árboles grandes no fallecen. Estos son los tres errores típicos a los que nos enfrentamos cuando se lleva a cabo un análisis de dinámica poblacional en especies con transiciones poco comunes.
...
## Ejemplo de un diagrama de un ciclo de vida usando el paquete `Rage`
...
## 1) Cargar helpers globales (save_plot_png, plc_save, grviz_save)
source("R/figuras_helpers.R")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
#| docx_fit: true
#| label: fig-cv11_rage_build
#| fig-cap: "Ciclo de vida generado con `Rage::plot_life_cycle()` a partir de la matriz `matA` con los estadios semillas, plántulas y adultos."
...
![Diagrama de ciclo de vida construido con el paquete `Rage`.](figs/CV11_Rage_plot.png){#fig-cv11}

```{r cv-rage-compare-show, echo=FALSE, message=FALSE, warning=FALSE}
