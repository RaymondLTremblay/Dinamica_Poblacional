...
Por: Aucencia Emeterio-Lara, Ernesto Mujica Benitez, Elaine González y Raymond L. Tremblay
...
Orchis_ficticia <- tribble(
  ~Num_ind, ~anio_1, ~anio_2, ~transición,
...
ficticia_matrix <- matrix(c(
  0.1428, 0.0000, 0.0000,
  0.2857, 0.5000, 0.0000,
  0.4285, 0.3333, 0.6667
), nrow = 3, byrow = TRUE)

mat_to_ft(ficticia_matrix)
```
...
La salida de la función `projection.matrix()` es una lista que contiene dos matrices:
...
```{r trans-orchis-ficticia, echo=FALSE}
Orchis_ficticia_por_ind <- tribble(
  ~Num_ind, ~anio_1, ~anio_2, ~fertilidad,
...
Esto significa que, en promedio, cada adulto añade 0.17 individuos a la clase "plantas pequeñas" en el segundo periodo de tiempo. Dicho de otro modo: de los seis adultos, solo uno aportó un recluta. La fecundidad de una matriz es un promedio por individuo, no un conteo, y por eso casi siempre es un número fraccionario.
...
Orchis_ficticia_df <- as.data.frame(Orchis_ficticia_por_ind) # Note que es importante convertir la tabla de datos a un data frame si es en formato de tibble.
...
Si desea obtener una matriz que combine tanto las transiciones como la fecundidad, puede utilizar el argumento `TF = FALSE` en relación con `matA`.
...
donde $\mathbf{U}$ contiene las **transiciones** entre estadios (supervivencia × cambio de estado), $\mathbf{F}$ las **fecundidades** (reproducción sexual: nuevos individuos incorporados al primer estadio por cada adulto reproductor) y $\mathbf{C}$ el **clonación** (reproducción asexual: nuevos individuos genéticamente idénticos a la planta madre). Mantener las tres matrices separadas permite analizarlas por separado y es la convención usada en bases de datos como COMPADRE.
:::
...
::: callout-warning
**La clonación no es fecundidad.** Aunque ambos procesos generan individuos nuevos, evolutivamente son distintos: la fecundidad sexual produce descendencia genéticamente diferente, mientras que la clonación (keikis, división de rizomas) produce *ramets* con el mismo genoma que la planta madre. Sumarlos en una sola matriz $\mathbf{F}$ confunde dos procesos demográfica y evolutivamente diferentes; conviene separar $\mathbf{C}$ para análisis correctos de selección, deriva genética y estructura poblacional.
:::
...
La matriz de transiciones de *Cypripedium calceolus* tiene una matriz de transiciones, una de fecundidad y una de clonación: `matA` = `matU` + `matF` + `matC`.
...
## Extracción de la matriz de Clonación

Note que la matriz de clonación es la siguiente.

Lo que se observa es que hay nuevos individuos en el tiempo $\text{t}_{i+1}$ que provienen de los individuos en el tiempo $\text{t}_i$. En este caso, son individuos que provienen de las etapas 4, 5 y 6 los que producen nuevos individuos (por clonación) en las etapas 3 y 4.

```{r Trans13}

matC <- matC(Cyp_cal_matC) # extraer la matriz de clonación a partir de la base de datos de COMPADRE
# matC() devuelve una lista de matrices (una por fila del CompadreDB).
# Aquí solo hay una fila, así que extraemos el primer elemento.
matc_mat <- as.matrix(as.data.frame(matC))
mat_to_ft(matc_mat)
```

### El ciclo de vida de *Cypripedium calceolus* con clonación

Ahora vemos el ciclo de vida de *Cypripedium calceolus* en este estudio, donde las líneas azules son las transiciones de una etapa a otra por clonación.

```{r}
#| docx_fit: true
#| label: fig-Trans14
#| fig-cap: "Diagrama del ciclo de vida generado con `grviz_save` a partir de la matriz `matC` con seis estadios para *Cypripedium calceolus*."
...
digraph {
  ...
  png = "images/Trans14_Cyp_calceolus_clon.png"
)
```

Los estudios sobre orquídeas que incorporan matrices de clonación son poco frecuentes, pero es fundamental considerar este proceso en los análisis de dinámica poblacional. La clonación puede tener un impacto significativo tanto en la dinámica de las poblaciones como en la evolución de las especies.

En este contexto, la clonación se entiende como una forma de reproducción asexual que puede modificar la estructura genética de la población y afectar su capacidad de adaptación frente a cambios ambientales.
