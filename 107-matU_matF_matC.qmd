...
- `matA = matU + matF + matC` se usa para calcular **$\lambda$** y proyecciones poblacionales completas.
...
::: callout-note
**Las entradas de `matA` pueden superar 1 cuando hay clonación.** En matrices puramente de transiciones (`matU`), cada columna suma a lo sumo 1 (incluyendo la mortalidad implícita). Pero al sumar `matU + matF + matC`, las entradas individuales pueden superar 1 porque un mismo individuo puede contribuir simultáneamente a varios destinos: sobrevivir y crecer (`matU`), producir plántulas sexuales (`matF`), y aportar *ramets* (`matC`). Una entrada \> 1 en `matA` no es un error — refleja que la columna ya no es una distribución de probabilidades de destino, sino una contabilidad de transiciones más reclutas.
:::
...
En orquídeas, cada pseudobulbo puede considerarse un *ramet*, mientras que la suma de todos los pseudobulbos conforma el *genet*. Existen enfoques para incorporar esta distinción en las matrices demográficas y, por ende, integrar el concepto de clonación en los estimados de adecuación [@mcgraw1996estimation].
...
## Extraer MPP con matriz de clonación

En el siguiente script se calcula la suma de todos los valores de la matriz de clonación (`matC`). Toda matriz cuya suma en `matC` sea mayor que 0 corresponde a una población en la que se registró clonación en al menos un estadio del ciclo de vida.

Conviene tener presente que el conteo es de **matrices**, no de especies: cada fila de COMPADRE es una matriz de una población y un periodo, y una misma especie puede aportar varias.

```{r mat5}
compadre <- readRDS("data/COMPADRE_2026.rds")

# Encontrar MPP con matC clonación no cero
...
as.data.frame(head(table(compadre$clonality))) |> flextable() # La primera fila cuenta las matrices cuya matC suma cero, es decir, sin clonacion registrada.
...
De las `r length(compadre$clonality)` matrices de la base, `r sum(compadre$clonality == 0, na.rm = TRUE)` no tienen ningún valor distinto de cero en `matC`, `r sum(compadre$clonality > 0, na.rm = TRUE)` sí registran clonación, y `r sum(is.na(compadre$clonality))` no traen `matC` disponible.
...
# Visualizar la base de datos de orquídea, se observa solamente dos matrices de clonación
...
#### Buscar la población que tiene `matC` máxima {.unnumbered}

En este código se evalúan todas las orquídeas que tienen una matriz de clonación y se filtra para la que tiene la suma de `matC` máxima. En este caso, es la misma población que se seleccionó en el script anterior.
...
#### Las etapas del ciclo de vida de *Cypripedium calceolus* son: {.unnumbered}
...
compmat_to_ft(Cyp_cal_2$mat[[1]])
```

------------------------------------------------------------------------

#### Visualizar la transición de las diferentes etapas por clonación solamente {.unnumbered}

Podemos visualizar las transiciones de las diferentes etapas por clonación. Use la función `plot_life_cycle()` de la librería `Rage` para visualizar el ciclo de vida. En este caso, las etapas son "Dormant", "Smallest", "Small", "Inter", "Large", "ExtraLarge" y la matriz de clonación es la `matC`. Si se reemplaza la matriz de clonación por la matriz `matA` se puede ver el ciclo de vida con todas las transiciones, fecundidad y clonación.
...
matC <- matC(Cyp_cal_2) # extraemos la matriz de clonación de esta especie/población

plc_save(
  matC[[1]], # Note aqui se seleciona solamente la matriz de clonación
...
### Cuántas familias de plantas incluyeron matriz de clonación en sus modelos
...
Bromeliaceae <- as.data.frame(Clonal_Species) %>%
  filter(Family == "Bromeliaceae") %>%
  select(SpeciesAuthor, Family, clonality) %>%
...
Seleccionamos una especie de bromelia, *Aechmea nudicaulis* [@sampaio2005ramet], que tiene una matriz de clonación. En este caso, la matriz de clonación es la `matC` y la matriz de transición es la `matU`; la matriz de fecundidad no está incluida en este modelo. Visualizamos ambas matrices.
...
::: callout-tip
**Regla práctica para distinguir crecimiento vegetativo de clonación.** Mientras los nuevos módulos (pseudobulbos, hijuelos, brotes de rizoma) **permanezcan conectados** a la planta madre y compartan recursos, se contabilizan como **crecimiento vegetativo** (van a `matU` como permanencia o crecimiento entre estadios). Solo cuando se **separan físicamente** y se vuelven ecológicamente independientes pasan a contabilizarse como **clonación** (van a `matC`). El criterio operativo es: ¿este módulo tiene un destino demográfico independiente del resto?
:::
...
::: callout-warning
**La mayoría de los modelos poblacionales de especies clonales omiten `matC`.** Janovsky et al. [@janovsky2017accounting] muestran que en COMPADRE la mayoría de las especies con reproducción clonal documentada **no** reportan una matriz `matC`, y muchos artículos que reconocen la clonación como un proceso importante tampoco lo incorporan formalmente al modelo. Esta omisión sesga las estimaciones de $\lambda$, oculta el efecto de la clonación sobre la diversidad genética, y limita análisis posteriores de selección y deriva.
:::
