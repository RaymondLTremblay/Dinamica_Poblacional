...
| Code & output | **Default Quarto rendering — all custom code/output styling was REVERTED on 2026-05-30** after repeated attempts (grey box, no-box, `#>` prefix, Rethinking input-box/output-plain via a Lua filter) caused problems — the Lua filter (`code-output-plain.lua`) flattened `.cell-output-display` and made flextables/figures vanish. So: no Lua code filter, no `comment` override, no code-block shading. `reference.docx` keeps only `SourceCode` left-aligned (`jc=left`, so code doesn't inherit the justified `Normal`) + JetBrains Mono. Input and output both render as ordinary code blocks. Don't re-attempt input-vs-output styling without a safe mechanism — Pandoc forces one style on all code blocks and touching `.cell-output-display` destroys tables/figures. |
