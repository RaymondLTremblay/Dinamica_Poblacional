...
| TOC / numbering depth | **Split by format; refined 2026-07-16.** HTML keeps `toc-depth: 3` (index + body). **Print (Typst/PDF + docx): the INDEX shows only two levels** (`toc-depth: 2`) so the printed TOC stays clean and consistent, **but the BODY keeps three numbered levels** (`section-numbering: "1.1.1"` for Typst, `number-depth: 3` for docx) — e.g. `3.4.1` still appears in the text (useful for internal cross-refs) but not in the TOC. Rationale: the original clutter complaint was about the printed *index*, not the body; three/four-level numbers in the TOC looked unprofessional. (Earlier 2026-07-15 version capped the body at two levels too; reverted 07-16 per author's resolution.) |
| Glosario / Índice de temas letters | Alphabet dividers (`A`, `B`, …) are `#### {letter} {.unlisted}` (H4 + `.unlisted`). H4 sits below every TOC depth used (HTML 3, print 2), so the letters stay out of **all** TOCs (only "Glosario"/"Índice de temas" appear) while remaining visible dividers; `.unlisted` is belt-and-suspenders for HTML. The Índice is generated — `scripts/build_topic_index.py` emits the H4+`.unlisted` form. |
