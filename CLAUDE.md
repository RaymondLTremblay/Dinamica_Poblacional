...
| TOC / numbering depth | **Split by format (2026-07-15):** HTML keeps `toc-depth: 3` (the collapsible web TOC benefits from detail). **Print formats are capped at two levels** — Typst/PDF: `toc-depth: 2` + `section-numbering: 1.1`; docx: `toc-depth: 2` + `number-depth: 2`. Deeper headings (H3/H4) still render in the body but appear **unnumbered** and stay out of the printed TOC. Rationale: three/four-level numbering (e.g. `3.4.1`, `8.10.4`) looked cluttered and inconsistent in the printed proof; two levels is the professional-book norm. Short third-level heads (e.g. "Especies epífitas/terrestres") read best as unnumbered subheads under their `X.Y` parent. |
