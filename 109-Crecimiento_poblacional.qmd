...
Comenzamos por ingresar dos matrices de transición, una para cada año de monitoreo de una poblaciones de *Laelia speciosa*.Los datos provienen de Mariana Hernández-Apolinar y son sin publicar.   
...
plot_life_cycle(mat1.1, stages=estadios, fontsize = 5)
...
plot_life_cycle(mat1.2, stages=estadios, fontsize = 5)
...
Después de proyectar la población por 7 iteraciones, podemos ver el $\lambda$ esperado es muy similar al calculado anteriormente, ±11% de crecimiento. Nota el vector de estadios resulta en un cambio la proporciones drástico de los estadios, ya que la población se proyecta a partir de una población inicial de 10 individuos en cada estadio (25% en cada uno)  a ser dominado por una población por juvenil y plántulas. El crecimiento poblacional se mide evaluando la suma de los individuos en cada estadio por año proyectado sobre la cantidad de individuos en el año anterior. Por ejemplo, en el primer año se espera que la población crezca de 40 individuos a 44 individuos, lo que representa un crecimiento de 10% (56.8/40=1.42). En el segundo año se espera que la población crezca de 56.8 a 70.2 individuos, lo que representa un crecimiento de 23% (70.2/56.8=1.24). 
...
Siempre se puede visualizar el crecimiento poblacional proyectado por año, usando la función **plot** en el paquete **popbio**. En este caso, se observa que la población crece de forma exponencial, lo que es consistente con el valor de $\lambda$ calculado anteriormente. Usan este mismo codigo y cambian el número de iteraciones para ver como cambia la proyección de la población. Que pasa si cambian el número de individuos en cada estadio inicial? ¿Qué pasa si cambian la la cantidad de iteraciones/time?

```{r}
popdemo::project(mat1.1, vector=c(10, 10, 10, 10), time=7) |> plot(, 
  main="Proyección de la población 1, año 1", 
  xlab="Años", ylab="Tamaño poblacional", 
  lwd=2, ylim=c(0, 130))
...
x<-boot.transitions(pop1.1, 1000, sort=stages) # la boot.transition seleciona al azar la cantidad de individuos en cada estadios: función esta en el paquete "popbio"
