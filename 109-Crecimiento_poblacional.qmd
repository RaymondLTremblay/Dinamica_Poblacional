...
## Por: Tamara TicktinMariana Hernández Apolinar
...
Comenzamos por ingresar dos matrices de transición, una para cada año de monitorio de una poblaciones de *Laelia speciosa*. Los datos provienen de Mariana Hernández-Apolinar y son sin publicar.
...
En el primer año se observa que la población tiene un valor de $\lambda$ \> 1.108, lo que indica que la población crecerá a largo plazo de aproximadamente de 11%. En el segundo año, el valor de $\lambda$ también es mayor de 1, con un estimado de 1.08 o sea de crecimiento de 8%, como es menor que el del primer año, indica que la tasa de crecimiento poblacional disminuyó de un año a otro.
...
En esta sección demostramos el crecimiento esperado por año, usando la función **pop.projection** en el paquete **popbio**. Esta función requiere que se especifique la matriz de transición, la cantidad de individuos en cada estado y el número de años a proyectar. En nuestro caso, proyectamos 5 años.

El resultado es una listas de resultados

-   el $\lambda$ por los años proyectados
-   la distribución estable de la población
-   la cantidad de individuos esperado por cada estapas en cada año proyectado
-   el tamaño poblacional esperado por cada año proyectado (la suma de los individuos en cada estadio)
-   el crecimiento intrínsico esperado por cada año proyectado

Después de proyectar la población por 7 iteraciones, podemos ver el $\lambda$ esperado es muy similar al calculado anteriormente, ±11% de crecimiento. Nota el vector de estadios resulta en un cambio de proporciones drástico de los estadios, ya que la población se proyecta a partir de una población inicial de 10 individuos en cada estadio (25% en cada uno) a ser dominado de seis años por juvenil y plántulas. El crecimiento poblacional se mide evaluando la suma de los individuos en cada estadio por año proyectado sobre la cantidad de individuos en el año anterior. Por ejemplo, en el primer año se espera que la población crezca de 40 individuos a 44 individuos, lo que representa un crecimiento de 10% (56.8/40=1.42). En el segundo año se espera que la población crezca de 56.8 a 70.2 individuos, lo que representa un crecimiento de 23% (70.2/56.8=1.24).
...
Siempre se puede visualizar el crecimiento poblacional proyectado por año, usando la función **plot** en el paquete **popbio**. En este caso, se observa que la población crece de forma exponencial, lo que es consistente con el valor de $\lambda$ calculado anteriormente. Usan este mismo código y cambian el número de iteraciones para ver como cambia la proyección de la población. Que pasa si cambian el número de individuos en cada estadio inicial? ¿Qué pasa si cambian la cantidad de iteraciones/time a un número grande?

```{r}
n=c(10,10,10,10) # Definir el número de individuos en cada estadio inicial
cambio=pop.projection(mat1.1, n, 7)

plot(cambio$pop.size, type="b", xlab="Año", ylab="Tamaño poblacional", main="Proyección de la población 1, año 1")



```

### Cambio en número de estadios por año proyectado

Se puede gráficar la cantidad esperada de individuos en cada estadio por año proyectado, usando la función **stage.vector.plot** en el paquete **popbio**. En este caso, se observa que la población se proyecta a ser dominada por juveniles y plántulas, lo que es consistente con el valor de $\lambda$ calculado anteriormente. El gráfico los visualiza por la proporciones de cada estadio en el periodo de tiempo correspondiente.

```{r}

n=c(10,10,1010,10) # Definir el número de individuos en cada estadio inicial
cambio=pop.projection(mat1.1, n, 7)

stage.vector.plot(cambio$stage.vectors, col=2:4, ylim=c(0, 0.6))
```

## Comparar $\lambda$ entre poblaciones

Nuestra segunda población de $Laelia speciosa$ se establece en un hábitat similar a la población 1 y fue monitoreada durante al mismo tiempo. Sin embargo, ésta fue sujeta a la extracción. Si comparamos los valores de $\lambda$ de nuestras dos poblaciones, podemos hacer inferencias sobre los efectos de la extracción. La población 2 no produjo frutos y tampoco se observaron plántulas. Debido a esto, fue necesario incluir el valor de 0.001 para la transición de la matriz adulta2-\> plántula y con esto asegurar que nuestra matriz de estudio sea irreducible y primitive.
...
Usando la función **boot.transitions** del paquete **popbio**, se puede calcular los intervalos de confianza de λ. Esta función requiere que se especifiquen los estadios de la matriz y el número de remuestreos. En nuestro caso, usamos 1000 remuestreos y especificamos los estadios como "pl", "j", "a1" y "a2".
...
Es apropiado tomar el tiempo de entender como se calcula $λ_e$ y cómo se interpreta. La aproximación de Tuljarpurkar es:
...
La variable de la ecuación incluye $\bar\lambda_1$, donde representa el log del valor propio promedio de las matrices de transición $\bar{\text{A}}$, donde es el promedio de cada elemento de las matrices. Si tenemos 4 matrices de transición, $\bar{\text{A}}$ es la matriz de transición promedio de cada elemento de esta matriz. Por ejemplo, si la transiciones de plántula a juvenil en la matriz 1 es 0.1, en la matriz 2 es 0.2, en la matriz 3 es 0.15 y en la matriz 4 es 0.05, entonces el valor promedio en la $\bar{\text{A}}$ de la transición de plántula a juvenil sería (0.1 + 0.2 + 0.15 + 0.05)/4 = 0.125. Por consecuencia el valor de de $\bar{\alpha}_{1,1}=0.125$ y así sucesivamente para cada elemento de la matriz.

El componente $\frac{\tau^2}{\bar\lambda_1^2}$ es una aproximado de la variación temporal del crecimiento poblacional por causa de estocasticidad temporal. En este caso, $\tau^2$ es el efecto de la sensitividad de $\lambda$ causado por los cambios en los elementos $\bar{\alpha}_{i,j}$. La covarianza $\text{Cov}$ mide la correlaciones entre las transiciones de estado $i$ a $j$ y $k$ a $l$ y se denota como $Cov(\alpha_{ij}, \alpha_{kl})$. Recuerda que no todos los valores en la matriz tienen el mismo impacto si fuesen a cambiar (capítulo de elasticidad). Se espera que algunos de los elementos tenga un impacto relativo y influencia de forma positiva y negativa sobre $\lambda$, por lo que la covarianza puede ser positiva o negativa. Por ejemplo, si la transición de plántula a juvenil aumenta, es probable que la transición de juvenil a adulto aumenta también en un buen año, y viceversa. Por lo tanto, la covarianza entre estas transiciones es positiva. Considera por ejemplo que la probabilidad de aumento en la transición de plántulas a juvenil es probable que haya una reducción en la cantidad de plántulas que siguen como plántulas en el próximo año, por lo que la covarianza entre estas transiciones es negativa.

Nota que aquí una vez más, se asume que las condiciones bajo las cuales se construyeron las matrices de transición son constantes en el tiempo. Por lo tanto, $λ_e$ no debe ser interpretado literalmente, ni tampoco como una herramienta predictiva. En adición los valores de IC se resta y se suma al $\lambda$ promedio obtenido para tener el rango de 95% de $\lambda$.
...
En nuestro caso, sabemos que es poco frecuente el reclutamiento episódico observado en el segundo muestreo de la población 1, pero también la mortalidad y regresión de adultos observado en ese muestreo es poco frecuente. Por lo tanto, establecimos que la probabilidad de ocurrencia de la matriz 1 (del año 1-año 2) fuera de 0.8, mientras que la probabilidad de ocurrencia de la matriz 2 (del año 2 a año 3) fuera 0.2. De esta forma uno puede similar el efecto de buenos y malos años en la dinámica poblacional, basado en el conocimiento de pratones de variación ambientales que afecte la dinamica de poblaciones. Vemos que $λ_e$ no cambia mucho en el ejercicio anterior.
