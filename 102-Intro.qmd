...
```{r}
#| label: fig-Pop-fig_1
#| fig-cap: "Trayectoria hipotética de una población invasora: un retraso inicial durante la colonización, una fase de crecimiento exponencial y una desaceleración al acercarse a la capacidad de carga."

# Curva logística hipotetica: reproduce las tres fases descritas en el texto.
# K = capacidad de carga, r = tasa de crecimiento, t0 = punto de inflexion.
invasion <- data.frame(tiempo = seq(0, 50, by = 0.5))
invasion$N <- 1000 / (1 + exp(-0.25 * (invasion$tiempo - 25)))

ggplot(invasion, aes(tiempo, N)) +
  geom_line(linewidth = 1.2, colour = "#009392") +
  geom_hline(yintercept = 1000, linetype = "dashed", colour = "grey50") +
  annotate("text", x = 3, y = 950, hjust = 0, size = 3.5,
...
Otra definición, de carácter evolutivo, considera una población como el conjunto de individuos entre los cuales ocurre flujo génico a lo largo de una generación. El tiempo generacional depende de la longevidad de la especie, que puede variar ampliamente. Por ejemplo, en las orquídeas epífitas de ramitas (*twig epiphytes*), el ciclo generacional puede ser de 3 a 4 años, mientras que en *Cypripedium calceolus*, una especie terrestre de zonas templadas, los individuos adultos pueden vivir más de 50 años. Cabe destacar que la «longevidad» no es la edad media de los individuos vivos —eso describe la estructura de edades de la población, no su tiempo de vida—, sino la edad a la que sobrevive una fracción pequeña de la cohorte (por ejemplo el 1 %), estimable a partir de la submatriz `matU` con funciones como `Rage::longevity()`. El único estudio que ha evaluado la longevidad en orquídeas es [@tremblay2000plant], limitado a especies del género *Lepanthes*.
