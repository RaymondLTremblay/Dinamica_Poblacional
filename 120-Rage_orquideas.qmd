...
- **`gen_time()` y `net_repro_rate()` requieren** que `matF` no sea idénticamente cero. Si la fecundidad solo aparece en `matC` (clonal), use `matC` en vez de `matF` o redefina su pregunta.
:::
...
- Sin embargo, hay casos en los que se muestrearon hasta `r max(SPECIES_O$n_populations)` poblaciones distintas para una misma especie.
...
ggplot(lambdaVals, aes(value)) +
  geom_histogram(fill = rlt_col1, colour = "white") +
  rlt_style_fill() +
  xlab("Lambda") +
  ylab("Frecuencia") +
  scale_x_continuous(n.breaks = 10) +
...
::: callout-warning
## Cuidado con la pseudorreplicación
...
La gráfica anterior puede ocultar la dispersión de los datos, especialmente cuando hay sesgos. Para abordar esto, se pueden añadir los datos originales a la figura. En este caso, se ha incorporado un símbolo de barra **\|** por cada valor de λ en la parte inferior de la gráfica. Al observar esta nueva visualización, se nota que la dispersión varía significativamente si se compara la representación basada en el promedio y el error estándar con la distribución real de los datos, representada por los **\|**. Esta figura permite apreciar mejor tanto la dispersión como la distribución de los datos. En comparación con la gráfica anterior, esta ofrece una perspectiva más completa, destacando los valores que se alejan del centro de la distribución.
...
Calcular esperanzas de vida y tiempos generacionales es la mitad fácil. La mitad difícil —y la que separa un análisis presentable de uno que sobrevive la revisión por pares— es **construir un argumento biológico** a partir de las métricas, no solo reportarlas.
...
Use `Rage::entropy_d()` sobre el subconjunto. ¿Las orquídeas son más iteróparas (entropía alta) o más semélparas (entropía baja)? Compare con grupos no-orquídeas usando otro `Family` en COMPADRE.
:::
