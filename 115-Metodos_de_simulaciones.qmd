---
description: "Simulaciones estocásticas y análisis de viabilidad poblacional (PVA): proyección de poblaciones bajo variabilidad ambiental y demográfica usando R."
image: "images/Serapias_cordigera_5_Giuseppe_Pellegrino.jpeg"
open-graph:
  description: "Simulaciones estocásticas y análisis de viabilidad poblacional (PVA): proyección de poblaciones bajo variabilidad ambiental y demográfica usando R."
twitter-card:
  description: "Simulaciones estocásticas y análisis de viabilidad poblacional (PVA): proyección de poblaciones bajo variabilidad ambiental y demográfica usando R."
...
Las simulaciones se construyen a partir de las transiciones demográficas introducidas en el capítulo de [Transiciones](105-Transiciones.qmd). Las trayectorias simuladas se interpretan en relación con la tasa de crecimiento poblacional discutida en el capítulo de [Crecimiento poblacional](109-Crecimiento_poblacional.qmd). A diferencia de los análisis [LTRE](#ERTV), que explican diferencias observadas, las simulaciones permiten explorar escenarios futuros no observados. Las simulaciones pueden extenderse naturalmente dentro de un marco bayesiano, como se describe en el capítulo de [Acercamiento bayesiano](108-Bayesian_PPM.qmd).
...
## Implementación de simulaciones poblacionales
...
- **Estocasticidad espacial:** variación entre poblaciones de la misma especie. Se modela seleccionando aleatoriamente entre matrices de distintos sitios.
- **Estocasticidad ambiental (temporal):** variación entre años o periodos, debida a que las condiciones del ambiente cambian y afectan a todos los individuos a la vez. Se modela seleccionando aleatoriamente entre matrices de distintos tiempos.
- **Incertidumbre paramétrica:** no sabemos con precisión cuánto valen los elementos de la matriz, porque se estimaron con pocos individuos. Se modela usando distribuciones beta/Dirichlet sobre los parámetros (enfoque bayesiano).

A estas tres hay que añadir la **estocasticidad demográfica** propiamente dicha: aunque conociéramos las probabilidades exactas, en una población de diez individuos el número de supervivientes es un conteo entero y aleatorio, y esa aleatoriedad no desaparece por tener mejores datos. Es la fuente que domina en poblaciones muy pequeñas y la que hace que el riesgo de extinción no sea despreciable aun con $\lambda > 1$.
...
El primer paso es definir las diferentes matrices, una por cada periodo de tiempo. En el siguiente script se enseña cómo entrar los datos para múltiples matrices en un solo objeto. Note que es una lista de matrices.

::: img-float
![*Serapias cordigera*. Foto: Giuseppe Pellegrino. Estudio demográfico: @pellegrino2014effects.](images/Serapias_cordigera_5_Giuseppe_Pellegrino.jpeg){#fig-serapias-cordigera-5-giuseppe-pellegrino style="float: right; margin: 10px;width: 50% ; "}
:::

Los datos provienen de un estudio de la dinámica de una población de *Serapias cordigera* de 1999 al 2012 evaluando el impacto antropogénico sobre la dinámica de la población [@pellegrino2014effects]. En este caso se emplean solamente 6 matrices de transición de la población de *S. cordigera* en 1999, tres de estas son poblaciones que tienen impacto antropogénico (A1, A2 y A3) y tres poblaciones naturales, sin impacto antropogénico (N1, N2, N3). Note que se construye una lista (`list()`) de matrices de transición, una por cada periodo de tiempo.
...
as.data.frame(head(Serapia.eq)) |> flextable() # vemos las primeras 6 simulaciones de los tamaños de muestra esperada por etapa
```

------------------------------------------------------------------------

### ¿Cuál es la distribución de la cantidad de individuos esperada?

Para observar cuántos individuos hubiese en el futuro de la población de *S. cordigera* si la especie se comporta como las matrices de transición. Se observa que las poblaciones tendrían pocos individuos, típicamente menos de 2-3 individuos en todas las etapas, sin importar si fuesen latente, plántula, rosetta o con flores.

::: img-float
![*Serapias cordigera*. Foto: Giuseppe Pellegrino. Estudio demográfico: @pellegrino2014effects.](images/Serapias_cordigera_7_Giuseppe_Pellegrino.jpg){#fig-serapias-cordigera-7-giuseppe-pellegrino style="float: left; margin: 10px;width: 40% ; "}
:::

```{r}
#| label: fig-sim4
#| fig-cap: "Histogramas (30 bins) del número de individuos simulados de *Serapias cordigera*, facetados por etapa (plántula, latente, rosetta, con flor) con escalas libres."
...
Serapia.eq %>%
  ...
  geom_histogram(bins = 30, fill = rlt_col1, colour = "white") +
...
as.data.frame(head(Serapia.uneq)) |> flextable() # vemos las primeras 6 simulaciones de tamaño de muestra
```
...
```{r}
#| label: fig-sim7
#| fig-cap: "Histogramas superpuestos (30 bins) del total de individuos por simulación de *Serapias cordigera* bajo escenarios uneq y eq."
...
- $r$ = log(crecimiento poblacional estocástico) \> 0 la población crece
- $r$ = log(crecimiento poblacional estocástico) \< 0 la población decrece
- $r$ = log(crecimiento poblacional estocástico) = 0 la población es estable

El crecimiento estocástico de las poblaciones de *S. cordigera* en 1999 es negativo, lo que sugiere que la especie está en declive. Pero también se puede ver que hay variación en el crecimiento poblacional de la especie. Se usa la función `stoch.growth.rate()`.

Simulaciones por selección de matriz con igual probabilidad

- el crecimiento es negativo y los intervalos de confianza también, dando confianza de que la especie está en declive.
...
- el crecimiento es negativo y los intervalos de confianza solapan el cero, lo que sugiere que hay variación en el crecimiento poblacional de la especie y que no hay evidencia de que la especie esté en declive ni aumentando (es estable). La interpretación aquí es que, si la especie está dominada por poblaciones con impacto antropogénico, tiene un crecimiento poblacional no negativo.
...
Para convertir el log(crecimiento poblacional estocástico) a λ se usa la siguiente fórmula $e^r$ donde $r$ es el log(crecimiento poblacional estocástico). Como ejemplo se muestra la metodología con las simulaciones con probabilidad desigual de selección de las matrices de transición.
...
#### Conversión de $r$ a $\lambda$ {.unnumbered}

λ — probabilidad desigual de selección

```{r sim10}

exp(SGR_uneq$sim) # matrices seleccionadas con probabilidad desigual
...
- Separamos las matrices de transición en dos grupos, uno para las poblaciones afectadas antropogénicamente y otro para las poblaciones naturales.
...
Asumimos una probabilidad igual de selección por cada simulaciones en ambos escenarios
...
Visualizar el tamaño poblacional esperado por etapa de la vida para las poblaciones afectadas antropogénicamente y naturales. Se observa que las poblaciones afectadas antropogénicamente tienen un tamaño poblacional más grande que las poblaciones naturales; las poblaciones con efecto antropogénico tienden a ser de mayor cantidad.

```{r}
#| label: fig-sim-diverging-colors
#| fig-cap: "Histogramas superpuestos (30 bins) del total de individuos por simulación de *Serapias cordigera* para los conjuntos Serapia_eq_N y Serapia_eq_A con tema minimal."
...
::: img-float
![*Serapias cordigera*. Foto: Giuseppe Pellegrino. Estudio demográfico: @pellegrino2014effects.](images/Serapias_cordigera_3_Giuseppe_Pellegrino.jpeg){#fig-serapias-cordigera-3-giuseppe-pellegrino style="float: left; margin: 10px;width: 50% ; "}

El objetivo de evaluar la variación en el tiempo es determinar si los efectos temporales son importantes para predecir los cambios en el crecimiento o la estabilidad poblacional. Seguimos con los datos de *S. cordigera*, pero ahora evaluamos cuál es el efecto de la variación en los parámetros de la matriz usando múltiples censos. Vemos el efecto en una de esas poblaciones con impacto antropogénico, solamente la población **A1**, en los 13 años de muestreo, desde 1999 hasta 2011, inclusive.
:::
...
El análisis evalúa la variación temporal en el crecimiento poblacional de la especie de *S. cordigera* en 1999 a 2012. Usando la función `stoch.projection()` se observa la variación en cantidad de individuos por etapa.
...
as.data.frame(head(Serapia_tiempo_eq)) |> flextable() # vemos las primeras 6 simulaciones de tamaño de muestra
```

Para evaluar el cambio de crecimiento poblacional en la especie de *S. cordigera* en 1999 a 2011 se usa la función `stoch.growth.rate()`.
...
#### Comparación entre la variaciones espacial y temporal {.unnumbered}

Comparar la variación temporal en el crecimiento poblacional de la especie de *S. cordigera* en 1999 a 2011 con la variación espacial.

Lo que se observa es que la variación temporal y la variación espacial es más o menos igual (el rango de variación, diff), pero que hay menos variación temporal que espacial. Por lo menos para los datos seleccionado de [@pellegrino2014effects] la variación temporal no es tan grande como la variación espacial y solapa la λ de 1, al contrario la variación espacial no solapa la λ de 1 (ambos CI son menores de 1, sugiriendo una reducción posible de las poblaciones). Note que no usamos todos los datos de Pellegrino [@pellegrino2014effects] pero un sub-grupo y por consecuencia el análisis es sesgado.
...
La otra ventaja de usar la función Dirichlet es que se basa en la distribución beta. Esta es una distribución donde tanto los parámetros como el promedio y los intervalos de confianza están limitados a 0 y 1. Otra manera de decir lo mismo es que no se pueden tener intervalos de confianza menores que 0 o mayores que 1. Hagan el ejercicio mental de tratar de entender un intervalo de confianza con una probabilidad de -0.10. Note que si uno usa una distribución normal, los intervalos de confianza pueden ser negativos o mayores de 1, lo que no tiene sentido biológico ni matemático en el contexto de probabilidad. De allí viene la ventaja de usar la distribución beta para calcular los parámetros y tener claro qué distribución se usa para estimar los parámetros.

### Seleccionar una población de *S. cordigera*

Se selecciona la población A3 de *S. cordigera* para hacer el análisis. Esta población es una de las poblaciones afectadas antropogénicamente, y tiene un tamaño de muestra pequeño, lo que hace que sea interesante para ver el efecto del tamaño de muestra en los parámetros de la matriz de transición.

```{r sim19}

n <- c(2, 0, 4, 99) # población inicial por etapa
names(n) <- c("latente", "plantula", "rosetta", "con flor")
# splitA() asigna POR OMISION toda la primera fila a la matriz de fecundidad
# (r = 1). Aqui la primera fila es "latente": son las transiciones hacia la
# latencia, no fecundidad. Esta matriz no contiene reproduccion sexual, asi que
# se le pasa una matriz logica de fecundidad vacia y toda A queda como matriz
# de transicion. Sin esto, las tres entradas de la fila 1 salian de la matriz de
# transicion y la mortalidad estimada de "con flor" subia de 0.114 a ~0.61.
sin_fecundidad <- matrix(FALSE, nrow = 4, ncol = 4)
Serapia_A3 <- splitA(Serapia$A3_1999, r = sin_fecundidad) # separar A3 en transición y fecundidad
```

------------------------------------------------------------------------

### Asignar una previa a los datos

En el siguiente paso vamos a asignar una previa a los datos. Estas previas provienen de Tremblay, no representan la visión de los autores del artículo. Se añade una previa también para la proporción de plantas que fallece en cada etapa (la quinta fila). La suma de cada columna tiene que ser igual a 1, así que esa quinta fila no se escribe a mano: se calcula como el complemento de las otras cuatro, y una comprobación con `stopifnot()` detiene el análisis si alguna columna no suma 1. Típicamente uno debería obtener la previa de la literatura o de un especialista en la biología de la especie antes de la recolección de datos, pero en este caso no se tiene información previa.

```{r sim20}

# Las cuatro primeras filas son las transiciones previas entre estadios.
...
# La quinta fila es la mortalidad, y NO se escribe a mano: es el complemento de
# cada columna. Asi cada columna suma exactamente 1 por construccion y no puede
# volver a descuadrarse al retocar una transicion. Antes estaba escrita a mano y
# las columnas de latente y plantula sumaban 0.90 y 0.20.
RLT_Serapia_previa <- rbind(
  RLT_Serapia_previa,
  1 - colSums(RLT_Serapia_previa)
)
rownames(RLT_Serapia_previa) <- c("latente", "plantula", "rosetta", "con flor", "muere")
colnames(RLT_Serapia_previa) <- c("latente", "plantula", "rosetta", "con flor")

stopifnot(all(abs(colSums(RLT_Serapia_previa) - 1) < 1e-12))
```

------------------------------------------------------------------------

### Obtener los intervalos de confianza (IC) para entradas de matriz individuales
...
- `priorweight` es el peso de la previa. Aquí se le da muy poca importancia, de modo que los datos de campo dominan el análisis. Cuánto pesa exactamente se explica en el recuadro siguiente, porque no es una cantidad fija.
- Regresa el posterior de la matriz de transición. Eso quiere decir que calcula los parámetros de la distribución beta para cada elemento de la matriz de transición tomando en cuenta tanto los valores de la matriz **TF** y la previa **P**.

&nbsp;

::: callout-important
**Cuánto pesa realmente la previa: no es un número fijo, depende del estadio.**

`priorweight` es un **multiplicador sobre el tamaño de muestra**, no una cantidad absoluta. La previa aporta `priorweight` $\times N$ observaciones equivalentes, y como cada estadio tiene su propio $N$, su peso cambia de una columna a otra. Con `n <- c(2, 0, 4, 99)` y `priorweight = 0.01`:

| Estadio  | $N$ observado | Peso de la previa |
|:---------|--------------:|------------------:|
| latente  |             2 |              0.02 |
| plántula |             0 |              0.00 |
| rosetta  |             4 |              0.04 |
...
La previa pesa unas cincuenta veces más en el estadio que ya tiene 99 individuos que en el que tiene 2. Conviene tenerlo presente, porque es al revés de lo que uno esperaría: la previa está justamente para sostener la estimación cuando faltan datos.

El caso extremo es la columna de plántula. Con $N = 0$ no hay datos, y la previa aporta 0.00 observaciones equivalentes: esa columna del posterior no está sostenida por nada y no debe leerse como una estimación.

**No confundir dos cosas distintas.** `priorweight` cambia *cuánto pesa* la previa; nunca cambia *su forma*. La forma la fija la matriz de previas, y esa sí debe ser siempre biológicamente realista. Una previa plana, que asigne la misma probabilidad a todas las transiciones, no lo es nunca: diría que una plántula tiene la misma probabilidad de florecer que de seguir siendo plántula. Ver el capítulo de [Datos sin sentido biológico](122-Impacto_de_Datos_sin_Sentido.qmd).
:::

&nbsp;

En el script vemos los intervalos de confianza de la matriz de transición de *S. cordigera* en 1999 para la población A3.
...
### Impacto de tamaño de muestra en los intervalos de credibilidad

&nbsp;

::: callout-warning
**La simulación no inventa información que no está en los datos.** Una población con solo 2 plántulas observadas tendrá intervalos de credibilidad muy amplios, y eso es lo correcto: refleja la verdadera incertidumbre del muestreo. Aumentar el número de simulaciones (`nreps`) no estrecha los intervalos — solo los estima con más precisión. Para estrechar intervalos se necesitan **más datos de campo** (más individuos marcados, más años de muestreo) o **previas informativas más fuertes** (justificadas por conocimiento experto). El enfoque bayesiano hace explícito este tradeoff entre datos y conocimiento previo.
:::

&nbsp;

Como ejercicio, vemos qué tipo de intervalos tendríamos si tuviésemos un tamaño de muestra de 500 individuos en la etapa de adulto. Note que, aunque los intervalos de credibilidad son más pequeños, no se debería asumir que no hay variación demográfica en la etapa de adulto, por procesos estocásticos.
...
A3 <- knitr::kable(sprintf("%.3f, %.3f, %.3f", p500, lcl, ucl))
...
```{r, warning=FALSE}
#| label: fig-sim25
#| fig-cap: "Cinco distribuciones beta (densidad en función de la probabilidad) para latentes, plántulas, rosettas, adultos y fallecidos, con líneas verticales punteadas, dispuestas en una cuadrícula 3x2."
...
plot_beta <- function(alpha, beta) {
  ...
  ggplot(data, aes(x, y)) +
    geom_line(colour = rlt_col1) +
    rlt_theme
}

# Distirbuciones beta
Latentes <- plot_beta(49.698, 50.292) +
  xlim(0, .1) +
  labs(
    title = paste("Distribución de latentes \n (alpha =", 49.698, ", beta =", 50.292, ")"),
    x = "Probabilidad", y = "Densidad"
  ) +
  theme(text = element_text(size = 6)) +
  geom_vline(xintercept = 0.498, linetype = "dashed", color = "#009392")

Plantulas <- plot_beta(0.000, 99.990) +
  xlim(0, .025) + ylim(0, .05) +
  labs(
    title = paste("Distribución de plántulas \n (alpha =", 0.000, ", beta =", 99.990, ")"),
    x = "probabilidad", y = "Densidad"
  ) +
  theme(text = element_text(size = 6)) +
...
Fallecidos <- plot_beta(11.385, 88.605) +
  xlim(.40, .8) +
  labs(
    title = paste("Distribución de fallecidos \n (alpha =", 11.385, ", beta =", 88.605, ")"),
    x = "probabilidad", y = "Densidad"
  ) +
  theme(text = element_text(size = 6)) +
  geom_vline(xintercept = 0.114, linetype = "dashed", color = "#009392")
...
La primera ventaja de usar métodos de análisis por simulaciones es que se pueden calcular intervalos de confianza. Si los intervalos de confianza de λ solapan el uno (1) (o solapan el 0 para $\log\lambda$) entonces no hay evidencia de que el crecimiento poblacional sea diferente de estable.
