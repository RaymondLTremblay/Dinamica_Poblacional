...
Si desea profundizar en estos índices, se recomienda consultar [@caswell2001matrix] y [@stott2011framework].
...
La estructura poblacional estable ($\mathbf{w}$, también llamada vector propio derecho) indica la proporción esperada de individuos en cada categoría de edad o estado cuando la población ha alcanzado el equilibrio a largo plazo. Por ejemplo, en *L. rubripetala* se espera que la estructura estable sea:

- Plántulas: `r round(stable.stage(Lr1)[1], 3)`
- Juveniles: `r round(stable.stage(Lr1)[2], 3)`
- Adultos no reproductivos: `r round(stable.stage(Lr1)[3], 3)`
- Adultos reproductivos: `r round(stable.stage(Lr1)[4], 3)`
...
ggplot(Wdf, aes(y = Wmax, x = Categorias)) +
  geom_bar(stat = "identity", fill = rlt_col1) +
  labs(
...
::: img-float
![*Spathoglottis plicata*. Foto: Tremblay. Estudio demográfico: @falcon2017quantifying.](images/Spathoglottis_plicata_Tremblay.jpeg){#fig-spathoglottis-plicata-tremblay style="float: left; margin: 10px;width: 40% ; "}
:::

El valor reproductivo ($\mathbf{v}$, también llamado vector propio izquierdo) indica la contribución esperada de cada categoría de edad o estado a la reproducción de la población.

En el caso de *L. rubripetala*, los valores reproductivos esperados son:

- Plántulas: `r round(reproductive.value(Lr1)[1], 2)`
- Juveniles: `r round(reproductive.value(Lr1)[2], 2)`
- Adultos no reproductivos: `r round(reproductive.value(Lr1)[3], 2)`
- Adultos reproductivos: `r round(reproductive.value(Lr1)[4], 2)`
...
El valor reproductivo también orienta las decisiones de manejo, pero conviene precisar qué pregunta responde y cuál no. El valor reproductivo mide cuánto vale demográficamente **un individuo** de cada estadio: cuánta descendencia futura, ponderada por $\lambda$, cabe esperar de él. Por eso sirve para comparar individuos. Por ejemplo:

- En *Spathoglottis plicata*, una orquídea terrestre invasora, retirar un individuo de un estadio con valor reproductivo alto elimina más contribución futura que retirar uno de un estadio con valor bajo.

- Por el contrario, si se busca remover individuos con el menor impacto posible sobre la población, conviene actuar sobre los estadios con valores reproductivos bajos.

Lo que el valor reproductivo **no** responde es en qué transición conviene intervenir para modificar $\lambda$. Esa es una pregunta sobre la matriz, no sobre los individuos, y la contestan la sensibilidad y la elasticidad, que se tratan en el capítulo de [Elasticidad y sensibilidad](111-Elasticidad.qmd). Un estadio puede tener un valor reproductivo alto y, a la vez, una elasticidad baja, si la transición que lleva a él ya está próxima a su máximo biológico.
...
Lo que $\rho$ mide es la rapidez con que se desvanece la influencia del segundo valor propio. Tras una perturbación, la desviación respecto a la estructura estable decae de forma proporcional a $\rho^{-t}$: cuanto mayor sea $\rho$, antes desaparece la huella de la estructura inicial. Por construcción $\rho \geq 1$, ya que el teorema de Perron y Frobenius garantiza que $\lambda_1 \geq |\lambda_2|$.

La siguiente gráfica compara esa caída para cuatro valores de $\rho$, incluido el de *L. rubripetala*:

```{r, message=FALSE, warning=FALSE}
#| label: fig-Indice13
#| fig-cap: "Influencia relativa del segundo valor propio en función del tiempo, para cuatro valores del índice de amortiguamiento. La curva de 1.21 corresponde a *L. rubripetala*; la línea discontinua marca el 10% de influencia restante."

library(ggplot2)

# Valores de rho a comparar. 1.21 es el de L. rubripetala, calculado arriba.
rho_valores <- c(1.05, 1.21, 1.5, 3.0)

tiempo <- seq(0, 50, by = 0.5)

# Tras una perturbacion, la desviacion respecto a la estructura estable
# decae de forma proporcional a rho^(-t).
amortiguamiento_df <- do.call(rbind, lapply(rho_valores, function(rho) {
  data.frame(
    Tiempo = tiempo,
    Influencia = rho^(-tiempo),
    Rho = factor(rho)
  )
}))

ggplot(amortiguamiento_df, aes(x = Tiempo, y = Influencia, color = Rho)) +
  geom_line(linewidth = 1.2) +
  geom_hline(yintercept = 0.1, linetype = "dashed", colour = "grey50") +
  labs(
    title = "Velocidad de convergencia según el índice \n de amortiguamiento",
    subtitle = "Cuánto pesa todavía la estructura inicial al pasar el tiempo",
    x = "Tiempo (en las unidades en que se construyó la matriz)",
    y = "Influencia relativa del segundo valor propio",
    color = "Índice (rho)"
  ) +
  rlt_style_colour() +
...
Las cuatro curvas muestran cuánto pesa todavía la estructura inicial a medida que pasa el tiempo. El punto de comparación práctico es cuándo cada una cruza el 10%, es decir, cuándo la influencia del segundo valor propio ha caído a la décima parte, que es el mismo criterio que usa `dr()` con `x = 10`:

- $\rho = 1.05$: unos 47 pasos. La convergencia es tan lenta que, con una matriz anual, el resultado asintótico no describe nada que el investigador vaya a observar.
- $\rho = 1.21$ (*L. rubripetala*): unos 12 pasos. Como la matriz es mensual, equivale aproximadamente a un año.
- $\rho = 1.5$: unos 6 pasos.
- $\rho = 3.0$: unos 2 pasos. La población olvida su estructura inicial casi de inmediato.

&nbsp;

::: callout-note
**Cómo leer el índice de amortiguamiento.**

- $\rho \geq 1$ siempre. Un modelo matricial de proyección es un sistema lineal en tiempo discreto y de primer orden, de modo que no tiene los regímenes «subamortiguado» ni «críticamente amortiguado» de un oscilador mecánico: esa clasificación pertenece a otro tipo de sistema y no se aplica aquí.
- $\rho$ cercano a 1: convergencia lenta, la estructura inicial se sigue notando durante muchos pasos de tiempo.
- $\rho$ grande: convergencia rápida.
- $\rho = 1$: la población no converge. Ocurre en matrices imprimitivas, cuyo ciclo de vida impone oscilaciones que nunca se amortiguan.
- Si $\lambda_2$ es un número complejo, la convergencia es oscilante: la desviación respecto a la estructura estable cambia de signo con un periodo de $2\pi/\theta$, donde $\theta$ es el argumento de $\lambda_2$. En *L. rubripetala* no sucede, porque $\lambda_2 = 0.8322$ es real.
:::
...
- Estructura completamente sesgada (solo plántulas): \~25 meses.
...
# Se calcula el vector propio derecho del valor propio dominante.
