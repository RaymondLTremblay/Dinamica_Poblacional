...
```{r}
life_expect_median <- function(matU, mixdist = NULL, start = 1L) {
  # Validate arguments
  if (!is.null(mixdist) && !is.null(start)) {
    stop("You cannot apply the mixing distribution and also specify a starting state.")
  }

  # Check that the MPM is valid
  checkValidMat(matU, warn_surv_issue = TRUE)

  # Convert start to numeric if it's a character
  if (!is.null(start)) {
    checkValidStartLife(start, matU, start_vec = TRUE)
    startNumeric <- if (inherits(start, "character")) match(start, colnames(matU)) else start
    if (!startNumeric %in% 1:nrow(matU)) {
      stop("The start value must match a stage name of matU, or be an integer between 1 and the number of stages.")
    }
  }

  # Compute life expectancy vector
  life_exp_vector <- Rage::life_expect_vec(matU)  # Assuming this returns life expectancy per stage

  # Apply mixing distribution or select starting stage
  if (!is.null(mixdist)) {
    # Weighted median (if needed)
    return(stats::weightedMedian(life_exp_vector, w = mixdist))
  } else {
    return(stats::median(life_exp_vector[startNumeric]))
  }
}
```

```{r}
life_expect_quartile_dispersion <- function(matU, mixdist = NULL, start = 1L) {
  # Validate arguments
  if (!is.null(mixdist) && !is.null(start)) {
    stop("You cannot apply the mixing distribution and also specify a starting state.")
  }
  ...
  # Compute life expectancy vector (per stage)
  life_exp_vector <- Rage::life_expect_vec(matU)  # Replace with your actual function

  # Apply mixing distribution or use starting stage
  if (!is.null(mixdist)) {
    Q1 <- stats::quantile(life_exp_vector, 0.25, na.rm = TRUE)
    Q3 <- stats::quantile(life_exp_vector, 0.75, na.rm = TRUE)
  } else {
    Q1 <- stats::quantile(life_exp_vector[startNumeric], 0.25, na.rm = TRUE)
    Q3 <- stats::quantile(life_exp_vector[startNumeric], 0.75, na.rm = TRUE)
  }

  # Calculate quartile dispersion
  return((Q3 - Q1) / (Q3 + Q1))
}
```
...
life_expect_quart_disp <- function(matU, mixdist = NULL) {
  # Validate MPM
  checkValidMat(matU, warn_surv_issue = TRUE)

  # Get stage-specific life expectancy
  le_vec <- life_expect_mean(matU, mixdist = mixdist, start = NULL)

  # Compute quartiles
  Q1 <- quantile(le_vec, 0.25, na.rm = TRUE)
  Q3 <- quantile(le_vec, 0.75, na.rm = TRUE)
...
```{r, eval = FALSE}
# Example usage of life_expect_median

median_life_exp <- life_expect_median(matU, start = 1L)


disp_Q <- life_expect_quart_disp(matU)
Q1
```
