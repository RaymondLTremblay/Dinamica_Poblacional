---
description: "Cómo calcular fecundidad en modelos matriciales de proyección poblacional: producción reproductiva, supervivencia y reclutamiento aplicado a orquídeas."
image: "images/Diagrama_fecundidad.png"
open-graph:
  description: "Cómo calcular fecundidad en modelos matriciales de proyección poblacional: producción reproductiva, supervivencia y reclutamiento aplicado a orquídeas."
twitter-card:
  description: "Cómo calcular fecundidad en modelos matriciales de proyección poblacional: producción reproductiva, supervivencia y reclutamiento aplicado a orquídeas."
...
- **Potencial:** lo que un individuo *podría* producir en condiciones óptimas (p. ej., número máximo de semillas si todas las flores son polinizadas).
- **Realizada:** lo que un individuo efectivamente contribuye a la siguiente generación, descontando aborto, fallas en polinización, semillas inviables, fallas de germinación y mortalidad temprana.

Para modelos demográficos solo es relevante la fecundidad **realizada**, expresada en las mismas unidades que el primer estadio del modelo.
:::
...
Antes de calcular nada hay que declarar **cuándo se hizo el censo respecto a la reproducción**, porque de eso depende qué debe contener la entrada de fecundidad. Es una decisión que no cambia los datos de campo, pero sí cómo se traducen a la matriz, y omitirla es una de las causas más frecuentes de que dos estudios de la misma especie no sean comparables.

&nbsp;

::: callout-important
**Declare el tipo de censo antes de construir la matriz F.**

En un **censo prerreproductivo** los individuos se cuentan justo antes de la ventana reproductiva. El primer estadio del modelo no son recién nacidos, sino individuos que ya sobrevivieron desde su origen, de modo que la entrada de fecundidad **tiene que incluir la supervivencia de la descendencia** hasta el censo siguiente.

En un **censo posreproductivo** los individuos se cuentan justo después de la reproducción. El primer estadio son los recién producidos, así que la entrada de fecundidad es simplemente **el número de descendientes producidos**, sin esa supervivencia.

**Los ejemplos de este libro asumen un censo prerreproductivo con intervalo de proyección de un año.** Se reconoce en que la fecundidad cuenta plántulas establecidas en $t_{+1}$ por adulto en $t_0$: la producción de semillas, la germinación y la supervivencia hasta plántula ya están dentro de **F**. Por eso la fecundidad de los ejemplos es una fracción o un número pequeño, y no un conteo de semillas.

Si su estudio cuenta frutos o semillas en el momento de producirlos, esos números **no** se pueden colocar directamente en una matriz cuyo primer estadio sea "plántula": faltaría multiplicarlos por la germinación y la supervivencia, que es exactamente el cálculo de la sección siguiente. Ver la entrada «Censo pre-reproductivo / post-reproductivo» del [Glosario](Apendice_Glosario.qmd).
:::
...
Diversos estudios ilustran este fenómeno. Por ejemplo, en la orquídea *Comparettia falcata* [@melendez2000reproduction], la polinización manual mostró una correlación positiva entre el número de flores producidas y el porcentaje de frutos abortados. En *Spiranthes spiralis*, un año con alta producción floral se tradujo en más del 80% de individuos en estado vegetativo al año siguiente [@willems2000flowering; @jacquemyn2010biological]. De manera similar, en la orquídea perenne *Cypripedium acaule* se observó una reducción en la probabilidad de florecer y en el tamaño foliar en la temporada posterior [@primack1998cost]. Otro caso se reportó en *Encyclia krugii*, donde el incremento en la producción de frutos se compensó en la siguiente temporada con menor crecimiento vegetativo (reducción en tamaño de hojas, número de flores, inflorescencias y mayor aborto de frutos) [@ackerman1989limitations]. El éxito de la polinización en sí mismo es variable y sensible al contexto: depende de la densidad de individuos en flor, que modifica las tasas de remoción y depósito de polinarios [@tremblay2007density], y puede verse reducido por perturbaciones de origen humano, como la cercanía a senderos transitados [@huang2009impact].
...
![Diagrama del proceso de fecundidad en orquídeas, mostrando la cadena de filtros sucesivos: producción floral, polinización exitosa, cuajado de fruto, producción de semillas con embrión, germinación y establecimiento de plántulas.](images/Diagrama_fecundidad.png){#fig-diagrama-fecundidad fig-alt="Diagrama esquemático del proceso de fecundidad en orquídeas, con la cadena de eventos desde la flor hasta el establecimiento de la plántula"}
:::
...
![Relación entre la cantidad de hojas y la probabilidad de floración en *Brassavola cucullata* en las islas Saba y San Eustaquio.](images/Cant_Hojas_Prob_Fr_Brassavola.jpg){#fig-cant-hojas-prob-fr-brassavola fig-alt="Gráfica de la probabilidad de floración como función del número de hojas en *Brassavola cucullata*, con curvas separadas para las poblaciones de Saba y San Eustaquio"}
:::
...
```{r}
#| docx_fit: true
#| label: fig-fec-life-cycle-1
#| fig-cap: "Ciclo de vida generado a partir de la matriz `matA_sem` con los estadios plántula, juvenil y adulto."
etapas <- c("plántula", "juvenil", "adulto")

plc_save(matA_sem, stages = etapas, fontsize = 5, png = "images/fec-life-cycle-1.png")
```
...
- cada adulto produce en promedio 1000 semillas,
- la tasa de germinación es 1%,
- y la tasa de supervivencia hasta plántula es 10%,

entonces la fecundidad en términos de plántulas se calcula como:

$$\text{Fecundidad} = 1000 \times 0.01 \times 0.10 = 1$$

Esto significa que, en promedio, cada adulto contribuiría con una plántula al siguiente periodo. Incluso estas dos tasas son generosas para una orquídea: en el campo el producto de germinación y supervivencia suele ser varios órdenes de magnitud menor, y por eso la fecundidad de un modelo matricial de orquídea casi siempre es una fracción.

Note que no hay que dividir entre el número de adultos de la población: las 1000 semillas ya son una cantidad *por adulto*, igual que la entrada de fecundidad de una matriz. La división entre el número de adultos solo corresponde cuando se parte de la producción **total** de la población, como en el ejemplo de la sección siguiente.

Por consecuencia la matriz cambia a:

```{r}
#| docx_fit: true
#| label: fig-fec-matA-pl
#| fig-cap: "Ciclo de vida generado a partir de la matriz `matA_pl` con fecundidad de 1 plántula por adulto."

matA_pl <- rbind(
  c(0.0, 0.0, 1.0), #  fecundidad 1 plántula en promedio por adulto
  c(0.5, 0.0, 0.0),
  c(0.0, 0.4, 0.0)
)
...
plc_save(matA_pl, stages = etapas, fontsize = 5, png = "images/fec-matA-pl.png")
```
...
```{r}
#| docx_fit: true
#| label: fig-fec-matA-pl2
#| fig-cap: "Ciclo de vida generado a partir de la matriz `matA_pl2` con fecundidad de 0.5 plántulas por adulto."
matA_pl2 <- rbind(
...
plc_save(matA_pl2, stages = etapas, fontsize = 5, png = "images/fec-matA-pl2.png")
```
...
1.  **Vía completa:** semillas producidas × tasa de germinación × tasa de supervivencia hasta plántula, dividido entre el número de adultos. Requiere mucha información empírica.
2.  **Vía intermedia:** producción de frutos × estimación de la viabilidad y germinación de semillas. Útil cuando se dispone de datos parciales.
3.  **Vía directa:** número de plántulas observadas en $t_{+1}$ dividido entre número de adultos en $t_0$. Es la más práctica cuando no hay datos de semillas/germinación, y la más usada en estudios de campo.
...
- Las plántulas registradas en $t_2$ provienen de los adultos en $t_1$.
- No hay inmigración desde otros sitios (o se modela explícitamente; ver [@pascarella1998hurricane]).
- No hay banco de semillas que aporte plántulas de cohortes anteriores.
- El periodo de muestreo coincide con la ventana en que las plántulas son visibles.
