...
A continuación, se muestran dos matrices de tipo Lefkovitch (matrices construidas a base de estadíos) para las poblaciones de *O. brachyandrum* y sus valores respectivos de λ en los dos hospederos, *Q. martinezii* (λ=1.3; IC: 1.11-1.45) y *Q. rugosa* (λ=0.98; IC: 0.87-0.99) (Cuadro 1). Notamos claramente que el valor de λ era mayor en *Q. martinezii* por lo que la matriz de esta especie fue la matriz de referencia para calcular los ERTV.
...
Para facilitar el entendimiento de cómo se calcula ERTV se creó la matriz con los términos de referencia (Cuadro 2). En la sección **Apéndice C** se encuentra una hoja de Excel para poder realizar los cálculos de ERTV. En esta sección se explica cómo se construye la matriz de términos y cómo se calculan las tasas vitales para cada una de las especies de hospedero.

Cuadro 2. Matriz de términos para el entendimiento de los ERTV.

Donde:

-   f= fecundidad
-   g= crecimiento o retrogresión
-   p= permanencia
-   s=supervivencia

Los subíndices se refieren a las transiciones entre estadíos. Nota que los parámetros de juvenil que retrogresan a plántula y y esa etapa produciendo plántulas se suman.
...
#### *Quercus matinezzi* elasticidad y sensitividad
...
#### *Quercus rugosa* elasticidad y sensitividad
...
How to convert a matrix unto a list with each element being a vector of the matrix row, append each row to the previous one

convert a matrix all elements to one row vector
...
mat_rug.vector <- as.vector(matrug)
mat_mart.vector <- as.vector(matmart)

```

Unir los vectores de las matrices rugosa y martinezii
...
Extract the matrix elements from the data frame and convert them into a list of matrices

```{r}
Ob_Atrt <- lapply(split(as.matrix(Unido[,1:16]), 1:2),  matrix, nrow=4, byrow=TRUE) 
...
# LTRE analysis for *Oncidium brachyandrum*
...
x_Ob
colnames(x_Ob) <-colnames(Unido)<-expression(a11, a21, a31, a41, 
                                             a12, a22, a32, a42, 
                                             a13, a23, a33, a43,
                                             a14, a24, a43, a44)

Ob_bp <- barplot(x_Ob[1:2,], beside=TRUE, ylim=c(-.2,.15), las=1, xlab="Transition", ylab="", xaxt='n')


mtext(expression(paste("Contribution of ", a[ij], "to variation in ", lambda)), 2, 3.5)
## rotate labels
text(Ob_bp[1,]-0.5, -.22, labels=colnames(x), srt=45, xpd=TRUE)
title(expression(italic("Oncidium brachyandrum")))
box()

```

Try to do ggplot figure

how to convert the matrix to a data frame for ggplot
...
Ob_df$Species <- c("Q. martinezii","Q. martinezii","Q. martinezii","Q. martinezii", "Q. rugosa","Q. rugosa","Q. rugosa","Q. rugosa")

colnames(Ob_df) <- c("a1i", "a2i", "a3i","a4i", "Species")
Ob_df$t_1 <- c("ai1", "ai2", "ai3", "ai4", "ai1", "ai2", "ai3", "ai4")


pivot_long <- tidyr::pivot_longer(Ob_df, cols = c("a1i", "a2i", "a3i", "a4i"), names_to = "Transition", values_to = "Contribution")

pivot_long$Transitions2 <- c("a11", "a21", "a31", "a41", "a12", "a22", "a32", "a42", "a13", "a23", "a33", "a43", "a14", "a24", "a34", "a44",
...
ggplot(pivot_long, aes(x = Transitions2, y = Contribution, fill = Species)) +
  ...
  labs(title = "Contributions of Transitions to Variation in Lambda",
       x = "Transition",
       y = "Contribution") +
  theme_minimal() +
...
Enlace sobre LTRE https://bookdown.org/cdorm/lefko3gentle/ltre.html
