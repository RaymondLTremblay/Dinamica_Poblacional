...
ggplot(LTRE.resultados.Martinezzi.Rugosa, aes(y=media.sens, x=razonvital, fill=razonvital))+
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  labs(title="Promedio de la Sensitividad", x="razonvital", y="contribución")+
  geom_hline(yintercept=0, linetype="dashed", color = "red")+
  xlab("Tasa vitales")
```
...
colnames(x_Ob) <-expression(a11, a21, a31, a41, 
                                             a12, a22, a32, a42, 
                                             a13, a23, a33, a43,
                                             a14, a24, a34, a44)

Ob_bp <- barplot(x_Ob[1:2,], beside=TRUE, ylim=c(-.05,.25), las=1, xlab="Transition", ylab="", xaxt='n')


mtext(expression(paste("Contribution of ", a[ij], "to variation in ", lambda)), 2, 3.5)
## rotate labels
text(Ob_bp[1,]-0.5, -.22, labels=colnames(x_Ob), srt=45, xpd=TRUE)
...
ggplot(pivot_long, aes(x = Transitions2, y = Contribution, fill = Species)) +
  ...
  labs(title = "Contribución de la variación  en las transiciones sobre lambda",
       x = "Transiciones",
       y = "Contribuciones") +
  theme_minimal() +
