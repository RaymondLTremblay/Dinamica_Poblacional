...
# as.data.frame evita errores de duplicated.default(col_keys) cuando el
# resultado viene con subclases de Rcompadre o columnas-lista.
...
# La metadata tiene muchas columnas; mostramos solo las primeras 4 como
# tabla y enumeramos el resto como texto debajo (mejor legibilidad en
# HTML/PDF/Word que una tabla extra-ancha).
# Convertir a data.frame antes de indexar para evitar problemas con
# columnas-lista o subclases de tibble que rompen flextable.
...
# Patrón seguro: as.data.frame + dplyr::select para evitar el error
# duplicated.default(col_keys) que dispara Rcompadre con flextable.
...
```{r Compr11}
# matA(Laeliasp) devuelve una LISTA con la matA de cada una de las cuatro
# filas (poblaciones/años). Mostramos la primera como flextable; las demás
# tienen la misma estructura. El print crudo de la lista se ve mal en Word.
...
```{r Compr12}
#| results: asis
# La segunda matriz es un objeto CompadreMat; `compmat_to_ft` lo renderiza
# como flextables legibles (matA, matU, matF, matC y las etapas) en vez del
# print crudo (ASCII de "matA:", "matU:", … que se ve mal en HTML/Word).
...
# Matriz promedio: produce un CompadreMat con matA, matU, matF, matC y la
# clasificación de estadios. `compmat_to_ft` lo renderiza como flextables
# legibles en HTML/PDF/Word (vs. el print() crudo que produce ASCII feo).
...
```{r Compr15}
#| results: asis
# Desviación estándar de los elementos de la matriz; mismo helper.
...
# mat_to_ft la convierte en flextable con las etiquetas de fila/columna.
