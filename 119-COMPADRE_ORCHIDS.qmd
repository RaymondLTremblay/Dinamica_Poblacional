...
| Quedarme solo con matrices "limpias" | `subset(cdb_flag(x), check_NA_A == FALSE & check_ergodic == TRUE)` |
...
- **`MatrixComposite`** indica si una matriz fue promediada o agrupada (*pooled*). Para análisis comparativo conviene quedarse con `MatrixComposite == "Individual"` para evitar pseudorreplicación.
...
La forma más sencilla es descargar el archivo `.RData` desde la página oficial: <https://compadre-db.org/Data/Compadre>. Al momento de la edición de este libro la versión pública más reciente es la 6.28.8.0 (agosto 2025). Conviene advertir, sin embargo, que COMPADRE se actualiza de forma continua y que **todas las cifras de este libro proceden de una única instantánea**, el archivo `data/COMPADRE_2026.rds`, que se carga en este capítulo y se reutiliza en los capítulos de `Rage` y de historia. Si el lector descarga una versión posterior, obtendrá números distintos de los impresos aquí; eso no es un error, sino el reflejo de que la base ha crecido. Por esa razón siempre debe declararse la versión de COMPADRE empleada al reportar resultados. Una vez descargado, se carga con `load()`:
...
Una vez cargada, conviene verificar el tamaño y la estructura. Al momento de este render la base tiene `r dim(compadre)[1]` filas (matrices) y `r dim(compadre)[2]` columnas (variables/metadatos).
...
Para evaluar si la base de datos contiene información sobre especies particulares de orquídeas,  se utilizará la función `cdb_check_species()`. Es relevante mencionar que se pueden evaluar múltiples especies de forma simultánea. Aquí se evaluó si la base contiene información sobre las especies *Epidendrum xanthinum*, *Epipactis gigantea* y *Caladenia latifolia*.
...
as.data.frame(cdb_check_species(compadre, especies)) |>
  flextable() |>
  ft_wide()
```
...
Seleccionaremos todas las matrices que pertenecen a la familia *Orchidaceae*.
...
La base contiene `r dim(index_O)[1]` matrices de transición de estados de poblaciones de orquídeas.
...
```{r Compr9}
# Evaluar la metadata de la base de datos de COMPADRE.
meta_O <- as.data.frame(cdb_metadata(index_O) |> head())
meta_O |>
  dplyr::select(1:7) |>
  flextable() |>
  ft_wide()
...
**Otras columnas disponibles en la metadata** (no mostradas arriba):
...
cols <- names(meta_O)[-(1:4)]
mat <- matrix(c(cols, rep("", (3 - length(cols) %% 3) %% 3)), ncol = 3, byrow = TRUE)
knitr::kable(mat, col.names = NULL, format = "markdown")
```
...
Observamos que hay 4 matrices en la lista: la matriz de transición de estados de poblaciones (`matU`), la matriz de fecundidad (`matF`), la matriz de clonación (`matC`) y la suma de las tres matrices (`matA` = `matU` + `matF` + `matC`).

```{r Compr10}
index_O %>% filter(SpeciesAccepted %in% c("Laelia speciosa")) -> Laeliasp # Extraer la información de la especie

meta_lae <- as.data.frame(cdb_metadata(Laeliasp) |> head(n = 3))
meta_lae |>
  dplyr::select(1:4) |>
  flextable() |>
  ft_wide()
...
**Otras columnas en la metadata de *Laelia speciosa*:** `r paste(names(meta_lae)[-(1:4)], collapse = ", ")`.

------------------------------------------------------------------------

### Extraer la matriz de transición de estados de poblaciones

En el siguiente script se muestra cómo extraer la matriz de transición de estados de poblaciones (`matA`) de *Laelia speciosa* y las etapas de estas matrices. Note que el objeto **mat** es una lista de matrices que incluye la matriz de transición (`matU`) de estados de una población, la matriz de fecundidad (`matF`), la matriz de clonación (`matC`) y la suma de las tres matrices (`matA` = `matU` + `matF` + `matC`); vea el [capítulo de Matrices de transición, fecundidad y clonación](#MatricesTFC).

```{r Compr11}
Laeliaspmat <- matA(Laeliasp)
mat_to_ft(Laeliaspmat[[1]])
```
...
Note que esta función es muy práctica porque extrae la matriz de transición de estados de poblaciones (`matA`), la matriz de supervivencia (`matU`), la matriz de fecundidad (`matF`) y la matriz de clonación (`matC`) sin necesidad de usar el código largo en adición de los nombres de las etapas.
...
- Para filtrar un género específico se puede usar el siguiente código.
  ...
  - Cada fila corresponde típicamente a una matriz de una población, un periodo o un análisis teórico distinto.
...
meta_epi <- as.data.frame(cdb_metadata(Epipactis) |> head(n = 3))
...
El género *Epipactis* devuelve `r dim(Epipactis)[1]` filas, todas de la misma especie y procedentes de dos estudios distintos.
...
mpm_promedio <- mpm_mean(mpms)
compmat_to_ft(mpm_promedio)
```
...
```{r Compr15}
#| results: asis
compmat_to_ft(mpm_sd(mpms))
```

Muchas veces es tedioso evaluar la base de datos completa de COMPADRE. Una manera de facilitar el acceso a los datos es aplanar la base con la función `cdb_flatten()`. Esta función toma todas las matrices de transición de estados de poblaciones (`matA`, `matU`, `matF`, `matC`) y las etapas de cada matriz y arma un `data.frame` (una tabla plana, con las matrices codificadas como texto) cómodo para revisar o exportar a CSV.
...
```{r Compr16}
Epi_flat <- cdb_flatten(Epi_Hens) # aplana la base: una fila por matriz

# dim() confirma la estructura: 49 filas (una por matriz) y columnas con las
# matrices (matA, matU, matF, matC) y las etapas de cada una.
dim(Epi_flat)
```

Ten en cuenta que `cdb_flatten()` codifica cada matriz como **texto** (pensado para exportar a CSV), no como una matriz numérica de R. Para obtener una matriz individual como objeto numérico —por ejemplo, la primera `matA`— conviene usar el accesor `matA()` sobre el objeto original y extraer el primer elemento con `[[1]]`:

```{r Compr16b}
# matA() devuelve la lista de matrices A del CompadreDB; [[1]] toma la primera.
mat_to_ft(matA(Epi_Hens)[[1]])
```
...
- **check_irreducible**: Determinar si `matA` es irreducible (ver isIrreducible). Algunos cálculos matriciales requieren que el MPP ("`matA`") sea irreducible. Los MPP irreducibles son aquellos en los que las tasas de transición parametrizadas facilitan el paso de todas las etapas a todas las demás etapas. Por el contrario, los MPP reducibles representan ciclos de vida incompletos donde no son posibles caminos desde todas las etapas a todas las demás etapas. Los MPP que son reducibles suelen ser biológicamente irracionales (pero no siempre), tanto en términos de la descripción de su ciclo de vida como de su dinámica proyectada. Por ello, hacen que algunos cálculos fallen. Sobre la relación entre irreducibilidad y ergodicidad, véase el recuadro de la sección *Ergodicidad e irreducibilidad*, más abajo.

- **check_primitive**: Determinar si `matA` es primitivo (ver isPrimitive). Una matriz primitiva es una matriz no negativa que es irreducible y tiene un solo valor propio de módulo máximo. Por lo tanto, esta verificación es redundante debido a la superposición con `check_irreducible` y `check_ergodic`.
...
Usando la función `cdb_flag()` se pueden evaluar todas las opciones en la base de datos de COMPADRE.

Usando la función `cdb_flag()` se identifican problemas potenciales en las matrices de transición de estados de las poblaciones. Esa función añade una columna por cada chequeo en la matriz. Note que para cada prueba anterior crea una columna con **TRUE** o **FALSE** para cada matriz en la base de datos.
...
meta_fla <- as.data.frame(cdb_metadata(flagged) |> head(n = 3))
meta_fla |>
  dplyr::select(1:7) |>
  flextable() |>
  ft_wide()
...
**Otras columnas en la metadata de las matrices marcadas:** 

```{r}
paste(names(meta_fla)[-(1:4)], collapse = ", ")
```
...
En los siguientes procedimientos se aplican filtros específicos a matrices de transición poblacional con el objetivo de evaluar dos propiedades fundamentales en el análisis demográfico: la ergodicidad y la irreducibilidad. Para ilustrar la relación entre ambas, se cuantificó el número de matrices que cumplen con cada propiedad.

&nbsp;

::: callout-important
**La irreducibilidad no es ni necesaria ni suficiente para la ergodicidad.** Es una relación que suele enunciarse mal, así que conviene fijarla con cuidado [@stott2010reducibility]:

- **No es suficiente.** Una matriz irreducible pero **imprimitiva** —aquella con más de un valor propio de módulo máximo— no converge: la población oscila indefinidamente con un periodo fijo en lugar de asentarse en una estructura estable. Lo que sí garantiza la ergodicidad es la **primitividad**: irreducible *y* con un único valor propio dominante en módulo.
- **No es necesaria.** Una matriz **reducible** puede ser perfectamente ergódica. Basta con que el bloque que contiene al valor propio dominante sea alcanzable desde todos los estadios; un ciclo de vida con un estadio terminal del que no se sale, por ejemplo, es reducible y aun así converge a una única estructura estable.

De ahí que `check_primitive` sea, en la práctica, la verificación decisiva, y que `check_irreducible` sirva sobre todo para detectar ciclos de vida mal parametrizados —caminos que el modelo no permite recorrer— más que para diagnosticar la convergencia.
:::

&nbsp;


#### Matrices ergódicas {.unnumbered}
...
meta_erg <- as.data.frame(cdb_metadata(Mat_ergodic) |> head(n = 3))
meta_erg |>
  dplyr::select(1:4) |>
  flextable() |>
  ft_wide()
...
**Otras columnas en la metadata de las matrices ergódicas:**

```{r}
paste(names(meta_erg)[-(1:4)], collapse = ", ")
```


#### Matrices irreducibles {.unnumbered}
...
meta_irr <- as.data.frame(cdb_metadata(Mat_irred) |> head(n = 3))
meta_irr |>
  dplyr::select(2, check_NA_A, 70) |>
  flextable() |>
  ft_wide() # nota que select(64, 70) se refiere a las posiciones de las columnas
```

**Otras columnas en la metadata de las matrices irreducibles:** 

```{r}
paste(names(meta_irr)[-(1:4)], collapse = ", ")
```


#### Matrices ergódicas e irreducibles {.unnumbered}
...
meta_ei <- as.data.frame(cdb_metadata(Mat_erg_irred) |> head(n = 3))
meta_ei |>
  dplyr::select(2, check_NA_A, 70, 69) |>
  flextable() |>
  ft_wide()
```

**Otras columnas en la metadata de las matrices ergódicas e irreducibles:**.

```{r}
paste(names(meta_ei)[-(1:4)], collapse = ", ")
```
...
Una vez identificadas las matrices que cumplen los chequeos relevantes para su pregunta, los siguientes pasos típicos son:
...
- **Comparar $\lambda$ entre poblaciones, especies o tratamientos** — ver los capítulos de [Crecimiento poblacional](109-Crecimiento_poblacional.qmd) y [LTRE](#ERTV).
