...
Para extraer una matriz individual de ese objeto —por ejemplo, la primera `matA`— se usa `[[1]]` (la indexación `Epi_flat$matA[1]` devuelve una lista de longitud 1, así que `[[1]]` saca la matriz misma):

```{r Compr16b}
# mat_to_ft convierte la matriz en una flextable con las etiquetas de fila y
# columna preservadas.
mat_to_ft(Epi_flat$matA[[1]])
```
