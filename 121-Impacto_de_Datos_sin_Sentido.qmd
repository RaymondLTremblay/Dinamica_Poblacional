---
bibliography: ftExtra.bib # Bibliografía para tablas tibble
---
...
```{r, sin_sentido_1, message=FALSE, warnng=FALSE}
...
::: img-float
![*Erycina crista-galli*. Foto: Diana Molina Ozuma](Species/Erycina%20crista-galli%20Diana%20Molina%20Ozuma.jpg){style="float: left; margin: 10px;width: 40% ; "}
:::
...
n=25
L=posteriorPRIORL <- MCmultinomdirichlet(c(.50*n,.2*n,.0*n,.3*n), c(.5,.2,0.0001,.3), mc=100000)
dfL=as.data.frame(L)
t(summary(dfL))
...
Transiciones=ddply(All_Data3, c("ind"), summarise,
            mean = round(mean(values),4), sd = round(sd(values),5),
...
n2=250
L=posteriorPRIORL <- MCmultinomdirichlet(c(.50*n2,.2*n2,.0*n2,.3*n2), c(.5,.2,0.0001,.3), mc=100000)
dfL=as.data.frame(L)
t(summary(dfL))
