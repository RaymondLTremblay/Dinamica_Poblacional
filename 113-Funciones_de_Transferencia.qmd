...
## Funciones de Transitoria y de Transferencia
...
-   Los elementos perturbados de la matriz, definidos como la estructura de perturbación, cuya magnitud se expresa mediante $\delta$.
...
Los análisis de elasticidad no lineal realizados por Tremblay y colaboradores [@tremblay2015stable] para Lepanthes rupripetala se desarrollaron mediante dos enfoques principales:
...
b)  Matriz de elasticidad lineal ($E$) derivada la MPP
...
Como se indicó previamente, se utilizarán los datos correspondientes a la población 1 de Lepanthes rupripetala, obtenidos del estudio de Tremblay y colaboradores [@tremblay2015stable]. La matriz denominada Lr1 describe las transiciones entre las siguientes categorías: plántula (PL), juvenil (J), no reproductivo (NR) y adulto reproductivo (AR).
...
Los resultados muestran que la permanencia en el estado no reproductivo ejerce un efecto no lineal sobre $\lambda$, con una respuesta drástica incluso ante perturbaciones pequeñas. La población responde en incrementos proporcionales de 0.01, desde el nivel mínimo hasta el máximo de perturbación. Según la gráfica, una perturbación menor provoca un cambio radical en el crecimiento poblacional. Mientras la sensibilidad lineal sugiere un descenso constante, la función de transferencia indica que, tras un descenso inicial, la población alcanza un punto de estabilización cercano a $\lambda \approx 0.95λ$.
...
Estas condiciones pueden generar inconsistencias, ya que las características de la especie analizada no siempre coinciden con los criterios establecidos por el autor del código. Por ejemplo, en la definición de elementos en la matriz $A$, el código asume que todos los valores del primer renglón corresponden a fecundidad ($F$). Sin embargo, en el caso de *L. rupripetala*, el elemento [1,1] representa una probabilidad de transición ($P$), no fecundidad. Este problema se resuelve incorporando la función `elementtype`, que permite especificar el tipo de cada elemento.
...
-   $\mathbf{v}^\top$\
    Es el **vector propio izquierdo transpuesto** asociado al eigenvalor dominante de la matriz de proyección. Pondera las etapas según su contribución al crecimiento.

-   $\hat{\mathbf{n}}_0$\
    Es el **vector inicial de población normalizado**, que indica la distribución inicial entre clases o etapas.

-   $\mathbf{w}$\
    Es el **vector propio derecho**, que describe la **distribución estable por etapas** cuando la población crece a la tasa asintótica.
...
Tremblay y colaboradores [@tremblay2015stable] usaron las funciones **tfa_inertia** y **tfam_inertia** para modelar la relación no lineal entre una perturbación, o manipulación en la estructura poblacional, y la inercia del crecimiento poblacional de *L. rupripetala*. Dado que la persistencia de una población puede evaluarse bajo distintos escenarios si se modifica un elemento de la matriz de transición, estos autores evaluaron la inercia poblacional considerando los siguientes supuestos:

a)  Un caso específico del ciclo de vida, por ejemplo el elemento con la elasticidad más alta.

b)  Cambios en todas y cada una de los elementos de la matriz $A$.
...
*Elasticidad no lineal a través de* **tfam_inertia**. En el cálculo de inercia poblacional se puede optar por evaluar los efectos de todas las entradas de la matriz con valor \>0. La función **matA** ofrece esta opción y su uso se ejemplificará a través de los tres escenarios faltantes (b, c y d).
...
*c) la estructura estable inicial*. En el siguiente recuadro se presenta el código del cálculo de la inercia poblacional para una población que se encuentra en su estructura estable y, por tanto, en equilibrio. Los resultados de la primera gráfica (extremo superior derecho) sugieren que la fecundidad tiene el mayor impacto positivo en la dinámica transitoria si la población se ecuentra en en equilibrio. En estas condiciones, duplicar la fecundidad aumenta significativamente el tamaño de la población a corto plazo.
