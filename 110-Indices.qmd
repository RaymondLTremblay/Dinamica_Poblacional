...
Aquí se añade un ejemplo de una matriz valores de reproducción al cuarto estadio, dando un resultado de "FALSE", lo que significa que la matriz es irreducible. Esto significa que no hay una ruta desde la categoría plántula a la categoría adulto reproductivo y o que no hay reproducción lo que no termina el ciclo de vida.
...
La matriz de transición poblacional debe ser primitiva, lo que significa que al elevar la matriz a altas potencias, se obtiene un único valor positivo (i.e. tasa de crecimiento poblacional). Esto es importante porque si la matriz no es primitiva, entonces no se puede esperar que la población alcance una estructura estable y una tasa de crecimiento constante a largo plazo. En el caso de *Lepanthes rubripetala* la matriz es primitiva, ya que al elevarla a altas potencias se obtiene un único valor positivo. Usando la función *isPrimitive* del paquete **popdemo** podemos comprobar que la matriz es primitiva, ya que el valor devuelto será "TRUE".
