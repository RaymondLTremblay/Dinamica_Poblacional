...
# La instantánea de COMPADRE que usa todo el libro. Se carga aquí porque las
# cifras del primer párrafo (familias, géneros, especies) se calculan en el
# render y necesitan el objeto antes de la sección de COMPADRE.
compadre <- readRDS("data/COMPADRE_2026.rds")
```

### Estudios realizados

La cantidad de estudios demográficos usando MPP es bastante impresionante y, al momento de la publicación del libro, incluye `r length(unique(compadre$Family))` familias de plantas, `r length(unique(compadre$Genus))` géneros y `r length(unique(compadre$SpeciesAccepted))` especies en la base de datos de COMPADRE, según la instantánea `COMPADRE_2026.rds` que se usa a lo largo del libro (véase [COMPADRE y orquídeas](119-COMPADRE_ORCHIDS.qmd)). La familia *Orchidaceae*, con `r length(unique(compadre$SpeciesAccepted[compadre$Family == "Orchidaceae"]))` especies, es una de las mejor representadas de la base (@tbl-familias-compadre), pero la gran mayoría de las familias tienen `r median(tapply(compadre$SpeciesAccepted, compadre$Family, function(x) length(unique(x))))` (mediana) o menos especies evaluadas. En la tabla del [Apéndice A](Appendix_A_Species_List.qmd) se presentan los estudios de dinámica poblacional en orquídeas que usaron MPP y que están incluidos en la base de COMPADRE. Se incluyen en la tabla algunas de las preguntas o conceptos principales que los autores evaluaron. En la gran mayoría de los trabajos se consideran múltiples hipótesis/ideas sobre la dinámica poblacional de las orquídeas. Para información específica sobre la metodología usada en cada estudio se recomienda revisar los trabajos originales.

### Los primeros estudios con Orquídeas

En esta sección se hará una revisión breve de los primeros estudios de dinámica poblacional en orquídeas que usaron MPP. Se presentarán algunos de estudios en orden cronológico y se resumirán las preguntas principales que los autores intentaron responder. Antes de los estudios mencionados abajo, Carl Olof Tamm [@tamm1948observations] publicó en 1948 un artículo sobre la dinámica poblacional de *Orchis mascula* y *Orchis sambucina* en Suecia. Estos trabajos son anteriores al desarrollo de las metodologías mencionadas en este libro. Vea el [capítulo sobre Tamm](118-Carl_Olaf_Tamm.qmd) para conocer un poco sobre sus trabajos.

&nbsp;

::: callout-note
**Carl Olof Tamm como punto de partida.** El estudio demográfico moderno de orquídeas comienza con Tamm (1948), décadas antes del desarrollo de las matrices de proyección. Tamm marcó individuos en el campo y los siguió por décadas, generando las primeras series temporales largas de orquídeas terrestres. Aunque sus análisis no usaron MPP, sus datos hicieron posible —y necesaria— la maquinaria matricial posterior. Para una semblanza completa de su trabajo, ver el [capítulo dedicado a Tamm](118-Carl_Olaf_Tamm.qmd).
:::
...
El problema de latencia —y de estimar los parámetros relevantes a esa etapa del ciclo de vida en la matriz de transición— es un tema recurrente en los estudios de orquídeas terrestres. No es claro si la latencia es algo común en la mayoría de las orquídeas terrestres o si es un componente de la historia de vida limitado a ciertas áreas geográficas o a determinados grupos taxonómicos.

Las primeras observaciones de no-emergencia en orquídeas son antiguas: ya Tamm [-@tamm1948observations] notó en la década de 1940 que muchas plantas marcadas desaparecían y reaparecían entre censos (ver el [capítulo sobre Tamm](118-Carl_Olaf_Tamm.qmd)). El reconocimiento de la *latencia prolongada* como un fenómeno general de las plantas con órganos subterráneos —y de su impacto sobre los estudios de monitoreo— se consolidó con Lesica y Steele [-@lesica1994prolonged], una referencia conceptual obligada aunque no se restringe a orquídeas. La respuesta metodológica al problema ha ido desde los modelos de captura-recaptura (Kéry y Gregg) hasta los acercamientos bayesianos (Tremblay y colaboradores en *Caladenia*), que se detallan en la siguiente sección.

Una pregunta de fondo sigue abierta: ¿la latencia es una estrategia adaptativa o un costo inevitable? Shefferson [-@shefferson2009evolutionary] propuso interpretar la latencia vegetativa como un rasgo de historia de vida adaptativo, una forma de *bet-hedging* frente a ambientes variables. Jäkäläniemi y colaboradores [-@jakalaniemi2011orchids] encontraron poca evidencia de un costo en la emergencia, mientras que Hurskainen y colaboradores [-@hurskainen2018vegetative] mostraron que los costos demográficos dependen del tamaño del individuo: las plantas grandes pagan un costo, las pequeñas no. La síntesis de Shefferson, Jacquemyn, Kull y Hutchings [-@shefferson2020demography] consolida la latencia como un fenómeno extendido y variable, central tanto para la estimación de $\lambda$ como para el riesgo de extinción. Conviene advertir que la terminología varía entre autores —*latencia vegetativa*, *dormancia prolongada*, *no-emergencia*—, lo que dificulta compilar revisiones bibliográficas verdaderamente exhaustivas.

¿Por qué es tan importante estimar la etapa de latencia? La dificultad es realmente determinar la supervivencia de los individuos: si los individuos pueden estar latentes por varios años, entonces la supervivencia no es observada y, por lo tanto, no se puede estimar directamente. Un individuo que no emerge en un año puede ser un individuo que ha muerto o uno que está latente. Por consecuencia, considerar muerto a un individuo que está latente sesga la estimación de la supervivencia. En los estudios evolutivos, no considerar correctamente esta etapa puede sesgar los estimados de esfuerzo reproductivo.
...
Los primeros trabajos de dinámica poblacional usando MPP, publicados en revistas revisadas por pares, libros o tesis enfocadas en orquídeas, comienzan a finales de los años 1980. La lista de trabajos de orquídeas que usaron MPP incluye:

- *Cleistes divaricata* [@gregg1989reproductive; @gregg1991variation]
- *Cypripedium acaule* [@cochran1992simple]
- *Laelia speciosa* [@hernandez1992dinamica] tesis de pregrado
- *Tolumnia variegata* [@calvo1990pollinator; @calvo1993evolutionary]
...
Es interesante que en estos primeros años hay 3 especies tropicales de las 8 estudiadas. Curiosamente, hoy hay más estudios de MPP en orquídeas tropicales (n = 35) que de especies templadas (n = 31).
...
Haciendo una búsqueda en Google Scholar para `"orchid" "population dynamics"`, se encontró un total de 3510 artículos. La mayoría de los artículos son revisiones o estudios que no usan MPP. Miramos los primeros artículos encontrados:

1940-1950

- Carl Olof Tamm publica el primer artículo sobre dinámica de orquídeas en *Botaniska Notiser* [@tamm1948observations]. Esta publicación tiene datos de *Orchis mascula* y *Orchis sambucina*, entre otras especies de plantas en Suecia. Vea el capítulo de Tamm para más información.
...
- Carl Olof Tamm [-@tamm1972survival] revisó sus análisis de dinámica poblacional de *Dactylorhiza sambucina*, *D. incarnata*, *Orchis mascula* y *Listera ovata*. Estos muestreos comenzaron en los años 1940 y se publicaron por primera vez en 1948 [@tamm1948observations].
...
- Katherine Gregg [-@gregg1989reproductive] —trabajo que sí emplea MPP y que por eso figura también en la lista anterior— estudió la dinámica poblacional de *Cleistes divaricata* en tres poblaciones en el este de los Estados Unidos. En este trabajo se presenta una revisión de la literatura sobre la dinámica poblacional de orquídeas y se discuten los métodos usados para estimar la dinámica poblacional. Se presenta un análisis de la dinámica poblacional usando MPP y se discute el impacto de la latencia en la estimación de los parámetros demográficos.

- Ricardo Calvo y Carol C. Horvitz [-@calvo1990pollinator] —igual que el anterior, este trabajo emplea MPP y aparece en la lista de la sección previa— estudian la dinámica poblacional de *Tolumnia variegata* en Puerto Rico. Se presenta un análisis usando MPP y se discute el impacto de la variación en transiciones sobre la adecuación demográfica, específicamente sobre el efecto de las limitaciones de polinizadores sobre la producción de frutos.
...
```{r}
#| label: tbl-familias-compadre
#| tbl-cap: "Las diez familias de plantas con más especies en la instantánea de COMPADRE que usa el libro, ordenadas de mayor a menor número de especies distintas. COMPADRE emplea los nombres conservados antiguos de dos de ellas: *Compositae* es *Asteraceae* y *Leguminosae* es *Fabaceae*."
#| message: false
#| warning: false

# Se agrupa TODA la base, no solo las orquídeas: agrupar `Lista_Orquidea` por
# familia daría una sola fila, porque ya está filtrada a Orchidaceae.
# Se construye un data frame simple con las dos columnas necesarias, en lugar
# de encadenar los verbos sobre el objeto CompadreDB.
SPECIES_Number <- data.frame(
  Family = compadre$Family,
  SpeciesAccepted = compadre$SpeciesAccepted
) %>%
  group_by(Family) %>%
  summarize(n_species = length(unique(SpeciesAccepted))) %>%
  arrange(desc(n_species))

pretty(head(SPECIES_Number, 10))
```

La mediana de especies por familia es `r median(SPECIES_Number$n_species)`, es decir, la mayoría de las familias están representadas por muy pocas especies; unas pocas familias concentran la mayor parte de los estudios.
